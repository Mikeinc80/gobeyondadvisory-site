...
def write_headers(out: Path, pages):
    """Emit a Netlify _headers file carrying a per-page CSP with JSON-LD hashes."""
    lines = []
    for meta in pages:
        url = href(meta["path"])
...
def build_site(key: str) -> int:
    ...
    for static in ("_headers", "_redirects", "netlify.toml"):
        f = src / static
        if f.exists():
