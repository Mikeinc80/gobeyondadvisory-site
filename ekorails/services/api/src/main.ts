...
async function serveStatic(
  requestPath: string,
): Promise<{ body: Buffer; contentType: string; nonce?: boolean } | null> {
...
async function preflight(): Promise<void> {
  ...
  const migrations = await withReadOnlyContext({ scope: 'system' }, (db) =>
    one<{ n: string }>(db, 'SELECT count(*)::text AS n FROM schema_migration'),
  );
  logger.info('Database reachable', { migrationsApplied: Number(migrations.n) });
  if (Number(migrations.n) === 0) {
    throw new Error(
      'No migrations are recorded. Run scripts/db-reset.sh before starting the service. ' +
  ...
  if (!ledger.trialBalanceBalanced || ledger.unbalancedJournals.length > 0) {
    throw new Error(
      'LEDGER INTEGRITY FAILURE at startup. The trial balance does not net to zero, or an ' +
  ...
  if (!chain.intact) {
    throw new Error(
      'AUDIT CHAIN BROKEN at startup. The hash chain does not verify from sequence ' +
...
async function main(): Promise<void> {
  ...
  const server = createHttpServer({
    ...
    verifyCsrf: (token, csrf) =>
      withReadOnlyContext({ scope: 'system' }, (db) => auth.verifyCsrf(db, token, csrf)),
