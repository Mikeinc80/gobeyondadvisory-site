...
export type ActorType = 'user' | 'partner' | 'job' | 'engine';

export interface TransitionDefinition {
  from: TransactionState;
  to: TransactionState;
  /** Machine name for this edge, used in audit records and in the UI. */
  event: string;
  /** Who may traverse it. Empty means only a non-user actor (partner, job, engine). */
  permittedPermissions: readonly Permission[];
  permittedActorTypes: readonly ActorType[];
  /** What must already be true. Checked before the edge is taken. */
  preconditions: readonly string[];
  /** What this edge does to the books. 'none' is a documented fact, not an omission. */
  accountingConsequence: string;
  /** Who is told. */
  notifies: readonly string[];
  /** Whether the actor must have re-asserted MFA within the step-up window. */
  requiresStepUp: boolean;
  /** Plain-English description for the Founder Learning Center walkthrough. */
  description: string;
}
...
export const TRANSITIONS: readonly TransitionDefinition[] = [
  {
    ...
    description:
      'The person who created the payment sends it for a colleague to authorise. They cannot ' +
  ...
  },
  {
    ...
    description:
      'A second person at the customer authorises the payment. The database refuses this step if ' +
  ...
  },
  {
    ...
    description:
      'A compliance analyst reviews the alerts the engine raised and clears the payment to proceed, ' +
  ...
  },
  {
    ...
    description:
      'A sanctions or serious alert suspends the payment pending investigation. It does not proceed ' +
  ...
  },
  {
    ...
    description:
      'Treasury issues a price. Until the customer accepts it, it is indicative and nothing is owed ' +
      'by anyone.',
  },
  {
    ...
    accountingConsequence:
      'OBLIGATION RECOGNITION: Dr customer funding receivable; Cr customer settlement payable, ' +
      'fee revenue, partner fees payable and any levy',
    notifies: ['treasury_operator', 'initiator'], requiresStepUp: true,
    description:
      'The customer accepts the price. This is the moment obligations come into existence in both ' +
  ...
  },
  {
    ...
    notifies: ['initiator'], requiresStepUp: false,
    description:
      'The customer is told where to send the funds. Note the destination is the partner institution, ' +
      'not EKORails.',
  },
  {
    ...
    accountingConsequence:
      'FUNDING RECEIPT: Dr partner funding account; Cr customer funding receivable',
    notifies: ['initiator', 'treasury_operator'], requiresStepUp: false,
    description:
      'The funds arrive at the origin partner. The ledger records them as sitting with the partner, ' +
  ...
  },
  {
    ...
    notifies: ['initiator'], requiresStepUp: false,
    description:
      'The customer did not fund in time. The obligation is reversed rather than deleted, so the ' +
  ...
  },
  {
    ...
    accountingConsequence:
      'FX CONVERSION and PARTNER POSITIONING: the obligation changes currency through FX clearing, ' +
      'and liquidity moves from the origin partner to the settlement partner',
    notifies: ['treasury_operator'], requiresStepUp: false,
    description:
      'Treasury converts the obligation and positions the funds. After this the FX clearing account ' +
  ...
  },
  {
    ...
    description:
      'The settlement instruction goes to the partner, carrying an idempotency key derived from the ' +
  ...
  },
  {
    ...
    accountingConsequence:
      'SETTLEMENT PAYMENT: Dr customer settlement payable; Cr partner settlement account',
    notifies: ['initiator', 'treasury_operator'], requiresStepUp: false,
    description:
      'The partner reports the payment as made. This discharges our obligation. It is not settlement ' +
  ...
  },
  {
    ...
    accountingConsequence:
      'SUSPENSE POSTING: the instructed amount moves to settlement suspense until the outcome is known',
    notifies: ['treasury_operator', 'finance_analyst'], requiresStepUp: false,
    description:
      'The most dangerous state in the system. We sent an instruction and never learned whether it ' +
  ...
  },
  {
    ...
    notifies: ['treasury_operator', 'initiator'], requiresStepUp: false,
    description:
      'The partner refused the instruction — insufficient liquidity, an invalid beneficiary, or its ' +
  ...
  },
  {
    ...
    accountingConsequence:
      'PARTIAL SETTLEMENT: the paid portion discharges the obligation; the shortfall goes to ' +
      'settlement suspense',
    notifies: ['treasury_operator', 'finance_analyst'], requiresStepUp: false,
    description:
      'The partner paid part of the instruction. The shortfall is not written off and is not treated ' +
  ...
  },
  {
    ...
    accountingConsequence:
      'RETURN RECEIPT: Dr partner settlement account; Cr returned funds. The original settlement ' +
      'journal is NOT reversed — the payment did happen.',
    notifies: ['initiator', 'treasury_operator', 'finance_analyst'], requiresStepUp: false,
    description:
      'The funds came back. The original payment is not erased: a return is a new event, and the ' +
      'ledger shows both.',
  },
  {
  ...
  },
  {
    ...
    notifies: [], requiresStepUp: false,
    description:
      'Our records, the ledger and the partner\'s statement all agree. Only then is the payment ' +
  ...
  },
  {
    ...
    notifies: [], requiresStepUp: false,
    description:
      'Some partners never send a beneficiary confirmation. Reconciliation can still complete, and ' +
  ...
  },
  {
    ...
    description:
      'Investigation established that no payment was made. Only now may the positioning be unwound.',
  },
  {
    ...
    accountingConsequence:
      'SETTLEMENT PAYMENT, and release of any suspense balance raised for the unknown outcome',
    notifies: ['treasury_operator', 'finance_analyst'], requiresStepUp: true,
    description:
      'Investigation established that the payment did go out after all. The suspense entry is cleared ' +
...
export class InvalidTransitionError extends Error {
  ...
  constructor(from: string, to: string, reference: string) {
    super(
      `No transition from "${from}" to "${to}" exists for ${reference}. ` +
...
export function findTransition(from: TransactionState, event: string): TransitionDefinition | undefined {
  return TRANSITIONS.find((t) => t.from === from && t.event === event);
}

export function transitionsFrom(state: TransactionState): TransitionDefinition[] {
...
export interface TransitionRequest {
  ...
  /** Whether the actor's step-up window is currently valid. */
  stepUpValid?: boolean;
  reason: string;
  evidence?: Record<string, unknown>;
  /** Posted by the caller inside the same transaction, linked to this transition. */
 ...
 */
export async function transition(db: Queryable, req: TransitionRequest): Promise<TransitionResult> {
  ...
  const txn = await maybeOne<{
    id: string; reference: string; state: TransactionState; organization_id: string;
    initiated_by: string;
  }>(
    db,
    'SELECT id, reference, state, organization_id, initiated_by FROM transaction WHERE id = $1',
    [req.transactionId],
  );
  ...
  if (!definition) {
    ...
    throw precondition(
      'INVALID_TRANSITION',
      `Cannot "${req.event}" a transaction in state "${txn.state}".`,
      { current_state: txn.state, available_events: available },
    );
  }
  ...
  if (!definition.permittedActorTypes.includes(req.actorType)) {
    ...
    throw forbidden(
      ...
      `This step cannot be performed by a ${req.actorType}.`,
      `edge ${definition.from}->${definition.to} permits ${definition.permittedActorTypes.join(', ')}`,
    );
  }
  ...
  if (req.actorType === 'user' && definition.permittedPermissions.length > 0) {
    const held = req.actorPermissions ?? new Set<Permission>();
    ...
    if (!ok) {
      ...
      throw forbidden(
        'PERMISSION_DENIED',
        'You do not have permission to perform this step.',
        `requires one of: ${definition.permittedPermissions.join(', ')}`,
      );
    }
  }
  ...
  if (definition.requiresStepUp && req.actorType === 'user' && req.stepUpValid !== true) {
    ...
    throw forbidden(
      'STEP_UP_REQUIRED',
      'Confirm your identity with your authenticator to perform this step.',
      `edge ${definition.event} requires step-up`,
    );
  }
  ...
  if (definition.event === 'business_approve' && req.actorUserId === txn.initiated_by) {
    ...
    throw forbidden(
      ...
      'You initiated this transaction and cannot also authorise it.',
      'maker-checker',
    );
  }
  ...
  const transitionRow = await one<{ id: string }>(
    db,
...
async function denyAndAudit(
  db: Queryable,
  txn: { id: string; organization_id: string; state: string },
...
export async function history(db: Queryable, transactionId: string): Promise<Array<Record<string, unknown>>> {
  return many<Record<string, unknown>>(
    db,
    `SELECT t.id::text AS id, t.from_state, t.to_state, t.actor_type, t.actor_role,
