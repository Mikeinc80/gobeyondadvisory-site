...
import { openExceptionCase, type Priority } from './exceptions.js';
...
export type RunType =
  | 'transaction_to_ledger'
  | 'ledger_to_partner_statement'
  | 'funding'
  | 'settlement'
  | 'fees'
  | 'currency_position';
...
export interface ReconciliationSummary {
  ...
  runType: RunType;
  businessDate: string;
  status: 'completed' | 'completed_with_breaks' | 'failed';
  itemsTotal: number;
  itemsMatched: number;
  itemsBroken: number;
  unexplainedAmount: string | null;
...
async function beginRun(
  db: Queryable, runType: RunType, businessDate: Date,
  options: { partnerId?: string | null; currency?: string | null; startedBy: string | null },
): Promise<RunContext> {
...
async function finishRun(
  ctx: RunContext, unexplained: Decimal | null, currency: string | null,
  ...
  await ctx.db.query(
    `UPDATE reconciliation_run
      ...
      WHERE id = $1`,
    [ctx.runId, status, ctx.items, ctx.matched, ctx.broken, unexplained?.toString() ?? null],
  );
  ...
  const run = await one<{ run_type: string; business_date: Date }>(
    ctx.db, 'SELECT run_type, business_date FROM reconciliation_run WHERE id = $1', [ctx.runId],
  );
 ...
 */
export async function reconcileTransactionToLedger(
  db: Queryable, businessDate: Date, startedBy: string | null,
): Promise<ReconciliationSummary> {
  const ctx = await beginRun(db, 'transaction_to_ledger', businessDate, { startedBy });

  const REQUIRED_JOURNALS: Record<string, string[]> = {
  ...
  const transactions = await many<{
    id: string; reference: string; state: string; organization_id: string;
    send_amount: string; send_currency: string; created_at: Date;
    journal_types: string[] | null;
  }>(
    db,
    `SELECT t.id, t.reference, t.state, t.organization_id, t.send_amount::text,
  ...
  for (const txn of transactions) {
    ...
    if (missing.length === 0) {
      ...
      await addItem(ctx, {
        ...
        detail:
          `Transaction is in state "${txn.state}" but the ledger is missing: ${missing.join(', ')}. ` +
...
export async function reconcileLedgerToPartnerStatement(
  db: Queryable,
  input: { partnerId: string; currency: string; businessDate: Date; startedBy: string | null },
): Promise<ReconciliationSummary> {
  ...
  const partner = await one<{ adapter_key: string; display_name: string }>(
    db, 'SELECT adapter_key, display_name FROM partner WHERE id = $1', [input.partnerId],
  );

  // Fetch the partner's own view.
  const statementLines = await settlementAdapter(partner.adapter_key).fetchStatement(
    { db, partnerId: input.partnerId, correlationId: randomUUID() },
    input.businessDate, input.currency,
  );

  // Persist it, so the comparison is reproducible later.
  const statement = await one<{ id: string }>(
    db,
    `INSERT INTO partner_statement (
  ...
  await db.query('DELETE FROM partner_statement_line WHERE statement_id = $1', [statement.id])
    .catch(() => { /* append-only guard: lines from an earlier run stay, which is correct */ });

  let lineNumber = 1;
  for (const line of statementLines) {
    await db.query(
      `INSERT INTO partner_statement_line (
  ...
  const ourLines = await many<{
    reference: string; transaction_id: string | null; organization_id: string | null;
    amount: string; posted_at: Date; transaction_reference: string | null;
  }>(
    db,
    `SELECT j.reference, j.transaction_id, j.organization_id, e.amount::text, j.posted_at,
    ...
    [input.partnerId, input.currency, input.businessDate.toISOString().slice(0, 10)],
  );
  ...
  for (const ours of ourLines) {
    ...
    if (!theirs) {
      ...
      await addItem(ctx, {
        ...
        detail:
          `Our ledger records a settlement of ${ourAmount.toString()} ${input.currency} for ` +
    ...
    if (!ourAmount.equals(theirs.amount)) {
      ...
      await addItem(ctx, {
        ...
        detail:
          `Amount difference on ${ours.transaction_reference}: our ledger says ${ourAmount.toString()}, ` +
  ...
  for (const line of statementLines) {
    ...
    await addItem(ctx, {
      ...
      detail:
        `${partner.display_name}'s statement shows ${line.amount.toString()} ${line.currency} for ` +
...
export async function reconcileFunding(
  ...
  const rows = await many<{
    transaction_id: string; organization_id: string; reference: string;
    expected_amount: string; received_amount: string | null; expected_currency: string;
    status: string; ledger_amount: string | null;
  }>(
    db,
    `SELECT f.transaction_id, f.organization_id, t.reference,
  ...
  for (const r of rows) {
    ...
    if (received.equals(inLedger) && received.equals(expected)) {
      ...
      await addItem(ctx, {
        ...
        detail:
          `Funding record says ${received.toString()} received but the ledger posted ` +
      ...
      await addItem(ctx, {
        ...
        detail:
          `Expected ${expected.toString()} but received ${received.toString()}. The ledger agrees with ` +
...
export async function reconcileSettlement(
  ...
  const rows = await many<{
    transaction_id: string; organization_id: string; reference: string; partner_id: string;
    instructed_amount: string; settled_amount: string | null; instructed_currency: string;
    status: string; idempotency_key: string; submission_count: string;
  }>(
    db,
    `SELECT s.transaction_id, s.organization_id, t.reference, s.partner_id,
  ...
  for (const r of rows) {
    ...
    if (submissions > 1) {
      await addItem(ctx, {
        ...
        detail:
          `${submissions} non-duplicate settlement submissions were recorded for ${r.reference} under ` +
    ...
    if (r.status === 'settled' && settled && settled.equals(instructed)) {
      ...
      await addItem(ctx, {
        ...
        difference: instructed, differenceCurrency: r.instructed_currency,
        detail:
          `Settlement instruction is "${r.status}" with no confirmed outcome. Until the partner ` +
...
export async function reconcileFees(
  ...
  const rows = await many<{
    reference: string; transaction_id: string; organization_id: string;
    quoted_fee: string; quoted_partner_fee: string; currency: string;
    posted_revenue: string | null; posted_partner_fee: string | null;
  }>(
    db,
    `SELECT t.reference, t.id AS transaction_id, t.organization_id,
  ...
  for (const r of rows) {
    ...
    if (quoted.equals(posted) && quotedPartner.equals(postedPartner)) {
      ...
      await addItem(ctx, {
        ...
        detail:
          `Quoted fees (EKORails ${quoted.toString()}, partner ${quotedPartner.toString()}) do not match ` +
 ...
 */
export async function reconcileCurrencyPosition(
  db: Queryable, businessDate: Date, startedBy: string | null,
): Promise<ReconciliationSummary> {
  ...
  for (const account of balances) {
    ...
    if (balance.isZero()) {
      ...
      await addItem(ctx, {
        ...
        detail:
          `FX clearing holds ${balance.toString()} ${account.currency}. This is an open currency ` +
  ...
  for (const row of tb) {
    if (!row.balanced) {
      await addItem(ctx, {
        ...
        detail:
          `THE LEDGER DOES NOT BALANCE in ${row.currency}. Debits ${row.totalDebits}, credits ` +
...
export async function runDailyReconciliation(
  ...
  const partners = await many<{ id: string; currency: string }>(
    db,
    `SELECT DISTINCT p.id, a.currency
...
export async function listRuns(
  ...
  return many<Record<string, unknown>>(
    db,
    `SELECT r.reference, r.run_type, r.business_date, r.status, r.items_total,
...
export async function getRun(db: Queryable, reference: string): Promise<Record<string, unknown> | null> {
  const run = await maybeOne<Record<string, unknown>>(
    db,
    `SELECT r.id, r.reference, r.run_type, r.business_date, r.status, r.items_total,
            r.items_matched, r.items_broken, r.unexplained_amount::text AS unexplained_amount,
            r.currency, r.started_at, r.finished_at, p.display_name AS partner_name
       FROM reconciliation_run r LEFT JOIN partner p ON p.id = r.partner_id
      WHERE r.reference = $1`,
  ...
  run['items'] = await many<Record<string, unknown>>(
    db,
    `SELECT internal_ref, internal_kind, internal_amount::text AS internal_amount, internal_currency,
       ...
       FROM reconciliation_item WHERE run_id = $1 ORDER BY result, internal_ref`,
    [run['id']],
  );
  return run;
}
