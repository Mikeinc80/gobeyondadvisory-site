...
export type SettlementOutcomeStatus =
  | 'accepted' | 'processing' | 'settled' | 'partially_settled'
  | 'rejected' | 'failed' | 'returned' | 'timeout' | 'duplicate_ignored';
...
export interface SettlementOutcome {
  ...
  /** Always true in this build. Written into the ledger and shown in the UI. */
  isSimulated: boolean;
  scenario: SimulationScenario;
  /** Set where we sent an instruction but did not learn the outcome. */
  outcomeUnknown: boolean;
  latencyMs: number;
}

export interface FundingConfirmation {
  status: 'confirmed' | 'partially_received' | 'awaiting' | 'failed';
...
export interface ScreeningOutcome {
  status: 'clear' | 'potential_match' | 'confirmed_match' | 'error' | 'provider_unavailable';
  hits: ScreeningHit[];
  provider: string;
...
export interface SettlementAdapter {
  ...
  submitSettlement(ctx: AdapterContext, submission: SettlementSubmission): Promise<SettlementOutcome>;
  confirmFunding(ctx: AdapterContext, expected: Decimal, currency: string, reference: string): Promise<FundingConfirmation>;
  fetchStatement(ctx: AdapterContext, date: Date, currency: string): Promise<StatementLine[]>;
}

export interface ScreeningAdapter {
  readonly key: string;
  readonly version: string;
  screen(ctx: AdapterContext, request: ScreeningRequest): Promise<ScreeningOutcome>;
}
...
export async function resolveScenario(
  ctx: AdapterContext, operation: string,
): Promise<{ scenario: SimulationScenario; parameters: Record<string, unknown>; directiveId: string | null }> {
  const directive = await maybeOne<{
    id: string; scenario: SimulationScenario; parameters: Record<string, unknown>;
    remaining_uses: number | null;
  }>(
    ctx.db,
    `SELECT id, scenario, parameters, remaining_uses
      ...
      LIMIT 1`,
    [ctx.partnerId, ctx.transactionId ?? null, operation],
  );

  if (!directive) return { scenario: 'success', parameters: {}, directiveId: null };

  if (directive.remaining_uses !== null) {
    await ctx.db.query(
      `UPDATE simulation_directive
...
export async function logIntegrationEvent(
  ...
  input: {
    ...
    outcome: 'success' | 'client_error' | 'server_error' | 'timeout' | 'duplicate_ignored' | 'rejected' | 'simulated_failure';
    latencyMs?: number;
...
export class DuplicateInstructionError extends Error {
  readonly code = 'DUPLICATE_INSTRUCTION';
  constructor(public readonly existingState: string, key: string) {
    super(
      `An instruction with idempotency key "${key}" is already ${existingState}. Refusing to send a ` +
...
export async function claimOutbound(
  db: Queryable,
  input: { partnerId: string; operation: string; idempotencyKey: string; transactionId: string | null },
): Promise<{ first: boolean; state: string; id: string }> {
  const existing = await maybeOne<{ id: string; state: string }>(
    db,
    `SELECT id, state FROM outbound_idempotency
      WHERE partner_id = $1 AND operation = $2 AND idempotency_key = $3`,
    [input.partnerId, input.operation, input.idempotencyKey],
  );

  if (existing) {
    await db.query(
      'UPDATE outbound_idempotency SET attempt_count = attempt_count + 1, last_sent_at = now() WHERE id = $1',
      [existing.id],
    );
  ...
  const created = await one<{ id: string }>(
    db,
    `INSERT INTO outbound_idempotency (partner_id, operation, idempotency_key, transaction_id)
     VALUES ($1,$2,$3,$4) RETURNING id`,
    [input.partnerId, input.operation, input.idempotencyKey, input.transactionId],
  );
  return { first: true, state: 'in_flight', id: created.id };
}

export async function resolveOutbound(
  db: Queryable, id: string, state: 'succeeded' | 'failed' | 'unknown',
): Promise<void> {
  await db.query(
    'UPDATE outbound_idempotency SET state = $2, resolved_at = now() WHERE id = $1', [id, state],
  );
}
...
export class SimulatedSettlementAdapter implements SettlementAdapter {
  readonly key = 'simulated_settlement_v1';
  readonly version = '1.0.0';

  async submitSettlement(
    ctx: AdapterContext, submission: SettlementSubmission,
    ...
    if (environment().liveFundsEnabled) {
      throw new Error(
        'SIMULATOR_IN_LIVE_MODE: the simulated settlement adapter was invoked while live funds are ' +
    ...
    if (!claim.first) {
      ...
      return {
        ...
        failureDetail:
          `Instruction ${submission.idempotencyKey} was already submitted and is ${claim.state}. ` +
    ...
    switch (scenario) {
      case 'partner_timeout': {
        ...
        outcome = {
          ...
          failureDetail:
            'The partner did not respond within the timeout. The instruction may or may not have been ' +
      ...
      case 'invalid_beneficiary': {
        ...
        outcome = {
          ...
          failureDetail:
            `The destination institution could not credit the account ending ${submission.beneficiaryAccountLast4}. ` +
      ...
      case 'compliance_failure': {
        ...
        outcome = {
          ...
          failureDetail:
            'The partner institution declined the instruction following its own compliance review. ' +
      ...
      case 'partial_settlement': {
        ...
        const settled = submission.amount.multiply(
          Decimal.fromString(fraction.toFixed(6)), 'down', submission.amount.scale,
        );
        ...
        outcome = {
          ...
          failureDetail:
            `The partner settled ${settled.toString()} of ${submission.amount.toString()} ${submission.currency}. ` +
      ...
      case 'returned_payment': {
        ...
        outcome = {
          ...
          failureDetail:
            'The destination institution credited and then returned the funds. A return is a new event, ' +
    ...
    await logIntegrationEvent(ctx, {
      ...
      outcome:
        outcome.status === 'timeout' ? 'timeout'
  ...
  async confirmFunding(
    ctx: AdapterContext, expected: Decimal, currency: string, reference: string,
  ): Promise<FundingConfirmation> {
    const { scenario, parameters } = await resolveScenario(ctx, 'funding.confirm');

    let result: FundingConfirmation;
   ...
   */
  async fetchStatement(ctx: AdapterContext, date: Date, currency: string): Promise<StatementLine[]> {
    const day = date.toISOString().slice(0, 10);
    const events = await many<{
      operation: string; response_payload: Record<string, unknown> | null;
      request_payload: Record<string, unknown> | null; occurred_at: Date;
      simulation_scenario: string | null; transaction_id: string | null;
    }>(
      ctx.db,
      `SELECT operation, response_payload, request_payload, occurred_at,
    ...
    for (const event of events) {
      ...
      if (typeof settled !== 'string' || eventCurrency !== currency) continue;
...
export class SimulatedScreeningAdapter implements ScreeningAdapter {
  readonly key = 'simulated_screening_v1';
  readonly version = '1.0.0';

  async screen(ctx: AdapterContext, request: ScreeningRequest): Promise<ScreeningOutcome> {
    ...
    const status: ScreeningOutcome['status'] =
      hits.length === 0 ? 'clear' : best >= 95 ? 'confirmed_match' : 'potential_match';
...
const SETTLEMENT_ADAPTERS = new Map<string, SettlementAdapter>([
  ['simulated_settlement_v1', new SimulatedSettlementAdapter()],
]);

const SCREENING_ADAPTERS = new Map<string, ScreeningAdapter>([
  ['simulated_screening_v1', new SimulatedScreeningAdapter()],
]);

export function settlementAdapter(key: string): SettlementAdapter {
  const adapter = SETTLEMENT_ADAPTERS.get(key);
  if (!adapter) {
    throw new Error(
      `UNKNOWN_SETTLEMENT_ADAPTER: "${key}" is not registered. Available: ` +
...
export function screeningAdapter(key: string): ScreeningAdapter {
  const adapter = SCREENING_ADAPTERS.get(key);
  if (!adapter) {
    throw new Error(
      `UNKNOWN_SCREENING_ADAPTER: "${key}" is not registered. Available: ` +
...
export function registeredAdapters(): { settlement: string[]; screening: string[] } {
  return {
...
export async function partnerByRole(db: Queryable, role: string): Promise<{
  id: string; code: string; display_name: string; adapter_key: string; status: string;
} | null> {
  return maybeOne(
    db,
    `SELECT id, code, display_name, adapter_key, status
