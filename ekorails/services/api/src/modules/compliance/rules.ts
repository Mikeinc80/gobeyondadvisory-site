...
export type RiskSeverity = 'low' | 'medium' | 'high' | 'prohibited';
...
export type RuleCategory =
  | 'customer_status' | 'beneficiary_status' | 'authorisation' | 'limits' | 'velocity'
  | 'corridor' | 'currency' | 'sanctions' | 'pep' | 'adverse_media' | 'jurisdiction'
  | 'industry' | 'behavioural' | 'documentation' | 'fraud' | 'related_party' | 'device';

/** The canonicalised document a rule set is evaluated against. */
export interface ComplianceInput {
  subjectType: 'transaction' | 'organization' | 'beneficiary';
  evaluatedAt: string;
  ...
  transaction?: {
    ...
    purpose: string;
    sourceOfFundsText: string;
    initiatedByUserId: string;
    initiatorHoldsInitiatePermission: boolean;
    initiatorIsAuthorisedSignatory: boolean;
    invoiceNumber: string | null;
    invoiceFingerprint: string | null;
    /** Other transactions in this organisation sharing the invoice fingerprint. */
    duplicateInvoiceMatches: Array<{ reference: string; state: string; createdAt: string }>;
    linkedDocumentRoles: string[];
    hasSourceOfFundsEvidence: boolean;
  };
  ...
  beneficiary?: {
    ...
    /** Other organisations' beneficiaries sharing this bank account fingerprint. */
    sharedAccountWithOtherOrgs: number;
    /** Beneficiaries this organisation has added in the trailing 7 days. */
    recentBeneficiaryAdditions: number;
    relationshipToSender: string;
    /** True where the beneficiary shares a UBO or director with the sender. */
    sharesControllerWithSender: boolean;
  };
  ...
  velocity: {
    ...
    pilotTotalAmount: string;
    currency: string;
    /** Transactions by this organisation in the trailing 24 hours, newest first. */
    recentAmounts: string[];
  };

  screening: {
    sanctions: { status: string; highestScore: number | null; matchedNames: string[] };
    pep: { status: string; isPep: boolean; categories: string[] };
    adverseMedia: { status: string; flagged: boolean; topics: string[] };
  };

  device: {
    ipHash: string | null;
    /** Distinct IP hashes seen for this organisation in the trailing 24 hours. */
    distinctIpCount24h: number;
    /** True where the session's IP hash has never been seen for this organisation. */
    newIpForOrganisation: boolean;
    failedLogins24h: number;
    knownFraudSignal: boolean;
  };
}
...
export interface RuleOutcome {
  triggered: boolean;
  message: string;
  /** Exactly the values the rule read. Recorded with the evaluation. */
  dataUsed: Record<string, unknown>;
  /** Optional override of the rule's declared severity for this evaluation. */
...
export interface RuleDefinition {
  ...
  evaluate: (input: ComplianceInput, params: Record<string, unknown>) => RuleOutcome;
}

const dec = (v: string | null): Decimal | null => (v === null ? null : Decimal.fromString(v));
const notTriggered = (message: string, dataUsed: Record<string, unknown> = {}): RuleOutcome =>
  ({ triggered: false, message, dataUsed });
 ...
 */
const DEFAULT_HIGH_RISK_JURISDICTIONS: string[] = [];
...
export const RULES: readonly RuleDefinition[] = [
  ...
  // -------------------------------------------------------------------------
  {
    ...
    riskAddressed:
      'Moving value for a business whose identity, ownership and legitimacy have not been ' +
    ...
    humanDecision:
      'None at this stage. The customer must complete onboarding; a compliance analyst then approves ' +
      'the organisation, not the individual transaction.',
    falsePositiveRisk:
      'Low. The only realistic false positive is a race where approval completes between the check ' +
      'and the submission, which the customer resolves by resubmitting.',
    policyBasis:
      'Customer due diligence before establishing a business relationship. Nigerian threshold and ' +
      'documentary specifics UNCONFIRMED pending the CBN filing.',
    evaluate: (input, params) => {
      const approved = (params['approvedStatuses'] as string[]) ?? ['approved'];
  ...
  },
  {
    ...
    parameters: {},
    riskAddressed:
      'A suspension is a live compliance or risk decision. Allowing a suspended customer to transact ' +
  ...
  },
  {
    ...
    parameters: {},
    riskAddressed:
      'Paying an unverified beneficiary. Beneficiary substitution is one of the most common ' +
    ...
    falsePositiveRisk:
      'Moderate operationally: a beneficiary whose details were edited legitimately is forced back ' +
    ...
    evaluate: (input) => {
      ...
      if (!b) return notTriggered('No beneficiary in scope.');
  ...
  // -------------------------------------------------------------------------
  {
    ...
    parameters: {},
    riskAddressed:
      'An account with read access initiating payments. Privilege creep inside a customer ' +
    ...
    evaluate: (input) => {
      ...
      if (!t) return notTriggered('No transaction in scope.');
      return {
  ...
  // -------------------------------------------------------------------------
  {
    ...
    parameters: {},
    riskAddressed:
      'Transacting outside a limit framework. A pilot without enforced limits is a pilot whose ' +
    ...
    automatedAction:
      'A missing limit is treated as a block, never as "unlimited". The transaction is held for ' +
      'manual review and cannot auto-clear.',
    humanDecision:
      'A Compliance Manager must either configure the approved limit from the filing or decline the transaction.',
    falsePositiveRisk:
      'None — but note this rule fires on EVERY transaction until the CBN filing supplies the limits. ' +
      'That is the intended behaviour, not a defect.',
    policyBasis:
      'Regulatory sandbox conditions ordinarily cap exposure per transaction, per customer and in ' +
  ...
  },
  {
    ...
    parameters: {},
    riskAddressed:
      'A single transaction breaching the exposure cap agreed for the pilot. A breach is a reportable ' +
    ...
    humanDecision:
      'None automatically. A limit increase requires a configuration change under maker-checker and, ' +
      'where the limit came from the filing, regulatory agreement.',
    falsePositiveRisk:
      'Low, but note that a limit expressed in a currency other than the send currency requires a ' +
    ...
    evaluate: (input) => {
      ...
      if (limit === null) return notTriggered('No per-transaction limit configured; handled by LIMIT_NOT_CONFIGURED.');
      if (c.limitCurrency !== t.sendCurrency) {
        return {
          ...
          message:
            `Limit is denominated in ${c.limitCurrency} but the transaction is in ${t.sendCurrency}. ` +
  ...
  },
  {
    ...
    evaluate: (input) => {
      ...
      if (!t || limit === null) return notTriggered('No daily limit configured or no transaction in scope.');
  ...
  },
  {
    ...
    evaluate: (input) => {
      ...
      if (!t || limit === null) return notTriggered('No monthly limit configured or no transaction in scope.');
      if (input.corridor.limitCurrency !== input.velocity.currency) {
  ...
  },
  {
    ...
    riskAddressed:
      'Deliberately breaking a payment into pieces that each sit just under a reporting or approval ' +
      'threshold. Structuring is an offence in its own right in most AML regimes, independently of the ' +
      'underlying funds.',
    triggerCondition:
      'Three or more transactions in 24 hours each between 90% and 100% of the per-transaction limit.',
    ...
    humanDecision:
      'A manager assesses whether the pattern has a commercial explanation and, if not, whether a ' +
      'suspicious activity report is required.',
    falsePositiveRisk:
      'Real. A business with a genuine invoice run near the cap looks identical to structuring. That is ' +
      'why the rule escalates for judgement rather than rejecting.',
    policyBasis:
      'Structuring / smurfing detection. Reporting obligation and threshold UNCONFIRMED pending the filing.',
    evaluate: (input, params) => {
      const t = input.transaction;
      const limit = dec(input.corridor.perTransactionLimit);
      if (!t || limit === null) return notTriggered('Structuring detection requires a configured limit.');
      ...
      const threshold = limit.multiply(
        Decimal.fromString(fraction.toFixed(6)), 'half_even', limit.scale,
      );
  ...
  // -------------------------------------------------------------------------
  {
    ...
    parameters: {},
    riskAddressed:
      'Transacting on a corridor that has not been confirmed by the regulatory filing. Until the ' +
    ...
    automatedAction:
      'The transaction cannot auto-clear compliance. It is held for a named analyst to review and approve.',
    humanDecision: 'An analyst approves the individual transaction, accepting that the corridor is provisional.',
    falsePositiveRisk:
      'None. This rule fires on every transaction until FD-002 is approved and the corridor is confirmed. ' +
      'That is the intended posture, and it means NO transaction in this build auto-clears compliance.',
    policyBasis:
      'Sandbox scope is defined by the approved corridor. Corridor UNCONFIRMED pending the CBN filing (FD-002).',
    evaluate: (input) => ({
  ...
  },
  {
    ...
    parameters: {},
    riskAddressed:
      'Settling a currency pair the pilot was not approved for, and for which no liquidity or partner ' +
      'arrangement exists.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    riskAddressed:
      'Providing a service to a sanctioned party. This is a strict-liability exposure in most regimes ' +
      'and the single most serious control failure this system can have.',
    triggerCondition:
      'The screening provider returns a potential or confirmed match against any screened party.',
    requiredEvidence:
      'The screening case, the provider response, the matched list entry and the analyst disposition.',
    automatedAction:
      'The transaction is suspended immediately — not merely queued — and cannot proceed to funding or settlement.',
    humanDecision:
      'A Compliance Analyst must dispose of the match as a true match or a false positive, with a written ' +
      'reason. A true match is escalated to a Compliance Manager for reporting.',
    falsePositiveRisk:
      'High and unavoidable. Name-based screening produces many false positives, especially for common ' +
    ...
    policyBasis:
      'Sanctions screening obligation. Applicable lists and the reporting route are UNCONFIRMED pending the ' +
  ...
  },
  {
    ...
    parameters: {},
    riskAddressed:
      'Elevated corruption and proceeds-of-crime risk where a controller of the business, or the ' +
    ...
    automatedAction:
      'The case is routed to enhanced due diligence and cannot be cleared by an analyst alone.',
    humanDecision:
      'A Compliance Manager approves or declines the relationship after enhanced due diligence, ' +
      'including source of wealth.',
    falsePositiveRisk:
      'Moderate. PEP lists include family members and close associates, and name matching is imprecise. ' +
      'A PEP relationship is not prohibited — it requires senior approval and closer monitoring.',
    policyBasis:
      'Enhanced due diligence for politically exposed persons. Nigerian definition and senior-approval ' +
  ...
  },
  {
    ...
    parameters: {},
    riskAddressed:
      'Publicly reported financial crime, fraud or regulatory action involving the customer or ' +
  ...
  },
  {
    ...
    riskAddressed:
      'Exposure to jurisdictions subject to call-for-action or increased-monitoring designations, or ' +
      'to a destination-market restriction.',
    triggerCondition:
      'The customer jurisdiction, beneficiary country or beneficiary bank country appears in the ' +
    ...
    falsePositiveRisk:
      'Low mechanically. The real risk is the opposite: an out-of-date list produces false NEGATIVES, ' +
      'which is why the list is a versioned rule parameter with an explicit review cadence.',
    policyBasis:
      'Enhanced measures for higher-risk jurisdictions. THE LIST IS DELIBERATELY EMPTY IN THIS BUILD: ' +
      'naming jurisdictions would assert a regulatory fact the filing has not supplied (FD-005).',
    evaluate: (input, params) => {
      const list = ((params['jurisdictions'] as string[]) ?? []).map((s) => s.toUpperCase());
      const candidates = [
  ...
  },
  {
    ...
    falsePositiveRisk:
      'Moderate. Self-declared industry codes are coarse and customers frequently pick the nearest ' +
    ...
    evaluate: (input, params) => {
      const list = ((params['industries'] as string[]) ?? []).map((s) => s.toLowerCase());
  ...
  // -------------------------------------------------------------------------
  {
    ...
    riskAddressed:
      'A business transacting far outside what it told us to expect. The gap is either a KYB failure ' +
    ...
    falsePositiveRisk:
      'High for growing businesses, and for customers who guessed when completing onboarding. This is a ' +
    ...
    evaluate: (input, params) => {
      ...
      if (input.organization.expectedTxnCurrency !== t.sendCurrency) {
        return notTriggered(
          `Declared size is in ${input.organization.expectedTxnCurrency}; transaction is in ${t.sendCurrency}. Not comparable without a recorded rate.`,
        );
      }
  ...
  },
  {
    ...
    riskAddressed:
      'A sudden step change in a customer\'s transaction size, which can indicate account takeover or ' +
  ...
  // -------------------------------------------------------------------------
  {
    ...
    riskAddressed:
      'Settling funds whose origin has not been evidenced. Without it there is no defence to a later ' +
      'allegation that the platform handled criminal property.',
    triggerCondition:
      'No source-of-funds document is linked, or the narrative is missing or too short to be meaningful.',
    ...
    falsePositiveRisk:
      'Low. The rule checks presence, not adequacy — adequacy is the analyst\'s judgement, which is why ' +
      'this holds for review rather than rejecting.',
    policyBasis:
      'Source of funds evidence in customer due diligence. Documentary standard UNCONFIRMED pending the filing.',
    evaluate: (input, params) => {
      const t = input.transaction;
      if (!t) return notTriggered('No transaction in scope.');
  ...
  },
  {
    ...
    riskAddressed:
      'Paying against a trade transaction with no underlying commercial documentation. Trade-based money ' +
    ...
    evaluate: (input, params) => {
      const t = input.transaction;
      if (!t) return notTriggered('No transaction in scope.');
      const required = (params['requiredRoles'] as string[]) ?? ['primary_invoice'];
  ...
  },
  {
    ...
    parameters: {},
    riskAddressed:
      'Paying the same invoice twice — through error, through internal fraud, or as a laundering ' +
      'technique that gives illegitimate funds a commercial-looking justification.',
    triggerCondition:
      'Another transaction in the same organisation shares this invoice fingerprint ' +
    ...
    falsePositiveRisk:
      'Moderate. Instalment payments against one invoice, and re-submissions after a failed attempt, both ' +
      'look like duplicates.',
    policyBasis: 'Trade-based money laundering red flag; double-payment operational control.',
    evaluate: (input) => {
      const t = input.transaction;
      if (!t) return notTriggered('No transaction in scope.');
      const live = t.duplicateInvoiceMatches.filter(
        (m) => !['cancelled', 'rejected', 'expired'].includes(m.state),
      );
      return {
  ...
  // -------------------------------------------------------------------------
  {
    ...
    riskAddressed:
      'One account collecting from several ostensibly unconnected customers — a classic mule and ' +
      'layering pattern, and also a signal of undisclosed common control.',
    triggerCondition:
      'The beneficiary bank-account fingerprint appears under another organisation\'s beneficiary.',
    requiredEvidence:
      'The account fingerprint match. Note the match is made on a keyed hash: the engine never ' +
    ...
    falsePositiveRisk:
      'Real. Group companies, shared agents and common suppliers legitimately share accounts. The rule ' +
  ...
  },
  {
    ...
    riskAddressed:
      'Business email compromise and account takeover, where an attacker adds a new beneficiary and ' +
    ...
    falsePositiveRisk:
      'High for genuinely new trading relationships and for a customer onboarding several suppliers at once. ' +
  ...
  },
  {
    ...
    parameters: {},
    riskAddressed:
      'Payments between entities under common control, which can move value with no genuine ' +
    ...
    falsePositiveRisk:
      'Low as a detection, but note that related-party payments are frequently entirely legitimate — ' +
  ...
  },
  {
    ...
    riskAddressed:
      'Account takeover. A payment instructed from an unfamiliar network, shortly after failed sign-in ' +
      'attempts, is the signature of a compromised credential being exercised.',
    triggerCondition:
      'The instructing session is on a network never seen for this organisation, or the organisation has ' +
      'an unusual number of distinct networks or recent failed sign-ins.',
    requiredEvidence:
      'Hashed network identifiers and the login-attempt history. Raw IP addresses are never stored.',
    automatedAction: 'Held for manual review; a security alert is raised in parallel.',
    humanDecision:
      'An analyst confirms the instruction with the customer through a known channel before release.',
    falsePositiveRisk:
      'High. Travel, remote working and mobile networks all produce new addresses routinely. Treated as ' +
    ...
    evaluate: (input, params) => {
      ...
      if (d.newIpForOrganisation) reasons.push('instruction from a network not previously seen for this organisation');
      if (d.distinctIpCount24h >= ipThreshold) reasons.push(`${d.distinctIpCount24h} distinct networks in 24h`);
      if (d.failedLogins24h >= failThreshold) reasons.push(`${d.failedLogins24h} failed sign-ins in 24h`);
      if (d.knownFraudSignal) reasons.push('device intelligence provider returned a fraud signal');
      return {
...
export function ruleByKey(key: string): RuleDefinition | undefined {
