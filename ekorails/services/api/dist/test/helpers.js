...
function findRepoRoot(start) {
    let dir = start;
    for (let i = 0; i < 8; i += 1) {
        if (existsSync(join(dir, 'scripts', 'db-reset.sh')))
            return dir;
        dir = join(dir, '..');
    }
...
export function connect() {
    ...
    const run = async (pool, context, fn) => {
...
export const SYSTEM = { scope: 'system' };
/** Asserts a promise rejects, and that the message or code matches. Returns the error. */
export async function expectRejection(promise, pattern) {
    try {
        await promise;
    }
    catch (error) {
        ...
        if (!pattern.test(err.message) && !pattern.test(code) && !pattern.test(err.name)) {
            throw new Error(`Rejected, but not in the expected way.\n  Expected to match: ${pattern}\n  ` +
...
export async function buildFixture(db) {
    const { seedRolesAndPermissions, seedCorridor, seedPartners, seedRules } = await import('../src/seed/reference.js');
    ...
    return db.asOwner(SYSTEM, async (q) => {
        ...
        const internal = await q.query(`INSERT INTO organization (display_code, legal_name, kind, onboarding_status)
       VALUES ('ORG-EKORAILS','EKORails LTD','internal','approved') RETURNING id`);
        const internalOrgId = internal.rows[0].id;
        const mk = async (email, name, roles, orgId) => (await createUser(q, {
        ...
        const makeOrg = async (code, name) => {
            const row = await q.query(`INSERT INTO organization (display_code, legal_name, kind, onboarding_status)
         VALUES ($1,$2,'customer','draft') RETURNING id`, [code, name]);
        ...
        const onboard = async (orgId, userId, name, industry) => {
            ...
            const profile = await q.query('SELECT id FROM organization_profile WHERE organization_id = $1 AND is_current', [orgId]);
        ...
        const makeBeneficiary = async (orgId, userId, name, iban) => {
            ...
            await q.query("UPDATE beneficiary SET created_at = now() - interval '45 days' WHERE id = $1", [result.beneficiaryId]);
...
export function actor(userId, role, permissions, stepUpValid = true) {
    return {
        userId, role,
...
export function settlementActor(userId, role, permissions) {
    return {
...
export async function createAndAdvance(db, fixture, options) {
    ...
    return db.asOwner(SYSTEM, async (q) => {
        ...
        const txn = await txnService.createTransaction(q, {
            ...
            sourceOfFunds: 'Trading revenue received into the operating account, evidenced by the attached statement.',
            invoiceNumber: invoice,
            documentLinks: links,
            initiatedBy: fixture.users.initiatorA,
        });
        if (options.stopAt === 'draft')
            return { ...txn, state: 'draft' };
        await txnService.submitForApproval(q, txn.id, actor(fixture.users.initiatorA, 'business_initiator', ['txn.initiate']));
        ...
        await q.query('UPDATE transaction SET fx_quote_id = $2, expected_receive_amount = $3 WHERE id = $1', [txn.id, quote.quoteId, quote.expectedReceivable]);
        await transition(q, {
        ...
        if (options.stopAt === 'quote_issued')
            return { ...txn, state: 'quote_issued' };
        await fx.acceptQuote(q, {
            quoteId: quote.quoteId, acceptedBy: fixture.users.approverA, organizationId: fixture.orgAId,
        });
        await settlement.acceptQuoteAndRecogniseObligation(q, txn.id, settlementActor(fixture.users.approverA, 'business_approver', ['fx.quote.accept']));
        await settlement.requestFunding(q, txn.id, settlementActor(fixture.users.treasury, 'treasury_operator', ['treasury.funding.review']));
        if (options.stopAt === 'awaiting_funding')
            return { ...txn, state: 'awaiting_funding' };
        await settlement.confirmFunding(q, txn.id, settlementActor(fixture.users.treasury, 'treasury_operator', ['treasury.funding.review']));
        if (options.stopAt === 'funding_confirmed')
            return { ...txn, state: 'funding_confirmed' };
        await settlement.prepareSettlement(q, txn.id, settlementActor(fixture.users.treasury, 'treasury_operator', ['treasury.settlement.route']));
        if (options.stopAt === 'ready_for_settlement')
            return { ...txn, state: 'ready_for_settlement' };
        if (options.scenario) {
            await q.query(`INSERT INTO simulation_directive (partner_id, transaction_id, operation, scenario, remaining_uses, created_by)
         VALUES ($1,$2,'settlement.submit',$3,1,$4)`, [fixture.partners.settlementInstitution, txn.id, options.scenario, fixture.users.admin]);
        }
        const submission = await settlement.submitSettlement(q, txn.id, settlementActor(fixture.users.treasury, 'treasury_operator', ['treasury.settlement.route']));
        if (options.stopAt === 'settled')
            return { ...txn, state: submission.finalState };
        if (submission.finalState === 'settled') {
            ...
            await settlement.complete(q, txn.id, settlementActor(fixture.users.finance, 'finance_analyst', ['recon.run']));
        }
        const final = await q.query('SELECT state FROM transaction WHERE id = $1', [txn.id]);
        return { ...txn, state: final.rows[0].state };
    });
}
export function timesReset() {
    return resetCount;
}
