...
import { Decimal, MoneyError, money, addMoney, subtractMoney, convert, RATE_SCALE, } from '../src/core/money.js';
import { describeEnvironment, EnvironmentError, RELEASE_GATES, assertLiveMoneyPermitted } from '../src/core/env.js';
import { hashPassword, verifyPassword, checkPasswordPolicy, encryptField, decryptField, generateTotpSecret, totpCodeForStep, totpStep, verifyTotp, fingerprint, canonicalJson, safeEqual, } from '../src/core/crypto.js';
import { ROLES, PERMISSIONS, permissionsForRoles, databaseScopeForRoles, maskingProfileForRoles, isReadOnlyRole, SEPARATION_RULES, } from '../src/auth/rbac.js';
...
describe('Money: fixed-precision arithmetic', () => {
    ...
    test('addition and subtraction are exact at scale', () => {
        ...
        for (let i = 0; i < 10_000; i += 1)
            total = total.add(Decimal.fromString('0.000001'));
    ...
    test('constructing from a value with too many decimals is REFUSED, not truncated', () => {
        assert.throws(() => Decimal.fromString('1.1234567'), (e) => e instanceof MoneyError && /Round explicitly/.test(e.message), 'silent truncation is how rounding bugs hide');
    });
    ...
    test('cross-currency arithmetic is refused', () => {
        assert.throws(() => addMoney(money('1.000000', 'NGN'), money('1.000000', 'USD')), (e) => e instanceof MoneyError && /Refusing to add NGN and USD/.test(e.message));
...
describe('Environment gating', () => {
    ...
    test('PRODUCTION with unmet gates REFUSES to start', () => {
        assert.throws(() => describeEnvironment({ EKORAILS_ENV_MODE: 'PRODUCTION' }), (e) => e instanceof EnvironmentError && /release gate/.test(e.message), 'the absence of an approval must be a startup failure, not a silent risk');
    });
    test('every single gate must be met; eight of nine is still a refusal', () => {
        const almost = { EKORAILS_ENV_MODE: 'PRODUCTION' };
        for (const gate of RELEASE_GATES.slice(0, -1))
            almost[gate.key] = 'true';
        assert.throws(() => describeEnvironment(almost), EnvironmentError);
    });
    test('a gate is met only by the exact string "true"', () => {
        const env = { EKORAILS_ENV_MODE: 'DEMO' };
        for (const gate of RELEASE_GATES)
            env[gate.key] = '1';
        assert.equal(describeEnvironment(env).unmetGates.length, RELEASE_GATES.length, '"1" must not count as met: a gate cannot be satisfied by a sloppy deployment script');
        for (const gate of RELEASE_GATES)
            env[gate.key] = 'TRUE';
    ...
    test('CONTROLLED_PILOT requires a recorded written authorisation reference', () => {
        assert.throws(() => describeEnvironment({ EKORAILS_ENV_MODE: 'CONTROLLED_PILOT' }), /EKORAILS_PILOT_AUTHORISATION_REF/);
...
describe('Cryptography', () => {
    ...
    test('the password policy weights length and rejects contextual words', () => {
        ...
        assert.equal(checkPasswordPolicy('Adaeze-Something-2026', { fullName: 'Adaeze Nwachukwu' }).acceptable, false, 'a password containing the user\'s own name must be refused');
        assert.equal(checkPasswordPolicy('folasade-quiet-2026X', { email: 'folasade@example.invalid' }).acceptable, false, 'a password containing the email local part must be refused');
    ...
    test('canonical JSON is stable regardless of key order', () => {
        assert.equal(canonicalJson({ b: 1, a: 2, c: { z: 1, y: 2 } }), canonicalJson({ c: { y: 2, z: 1 }, a: 2, b: 1 }));
    });
...
describe('Role and permission matrix', () => {
    ...
    test('every role permission exists in the permission catalogue', () => {
        for (const role of Object.values(ROLES)) {
            for (const permission of role.permissions) {
                assert.ok(permission in PERMISSIONS, `${role.code} references unknown permission "${permission}"`);
            }
        }
    });
    test('every role states what it explicitly cannot do', () => {
        for (const role of Object.values(ROLES)) {
            assert.ok(role.explicitDenials.length > 0, `${role.code} must state its explicit denials; an absent permission is an oversight waiting to happen`);
        }
    });
    test('a business initiator cannot approve, clear compliance or touch the ledger', () => {
        ...
        for (const denied of [
            ...
            assert.equal(permissions.has(denied), false, `business_initiator must not hold ${denied}`);
        }
    });
    test('a system administrator cannot reach transactions, compliance or the ledger', () => {
        ...
        for (const denied of [
            ...
            assert.equal(permissions.has(denied), false, `system_administrator must not hold ${denied}`);
        }
    });
    test('the auditor role is read-only: it holds no write permission at all', () => {
        ...
        const writeShaped = permissions.filter((p) => /\.(write|manage|approve|clear|initiate|route|post|suspend|configure|propose|file|raise)/.test(p));
    ...
    test('sensitive permissions are marked as such', () => {
        for (const key of [
            ...
            assert.equal(PERMISSIONS[key].sensitive, true, `${key} must be marked sensitive`);
        }
    });
});
// ---------------------------------------------------------------------------
describe('Settlement state machine', () => {
    ...
    test('there is no transition INTO draft: a transaction cannot be un-submitted', () => {
        assert.equal(TRANSITIONS.filter((t) => t.to === 'draft').length, 0, 'nothing may move a transaction back to draft');
    });
    test('terminal states have no outbound transitions', () => {
        for (const state of TERMINAL_STATES) {
            assert.deepEqual(transitionsFrom(state).map((t) => t.event), [], `terminal state "${state}" must have no way out`);
        }
    });
    ...
    test('a partner cannot take a compliance or approval edge', () => {
        for (const event of ['compliance_approve', 'business_approve', 'compliance_reject']) {
            const t = TRANSITIONS.find((x) => x.event === event);
            assert.equal(t.permittedActorTypes.includes('partner'), false, `${event} must not be reachable by a partner callback`);
        }
    });
    ...
    test('no edge exists that would skip compliance', () => {
        const skipping = TRANSITIONS.filter((t) => t.from === 'pending_business_approval' &&
    ...
    test('finality is never claimed anywhere in the machine', () => {
        for (const t of TRANSITIONS) {
            ...
            if (/final/.test(text)) {
                assert.match(text, /not (mean )?settlement finality|finality, which/, `"${t.event}" mentions finality without disclaiming it`);
            }
        }
    });
});
...
describe('Compliance rule catalogue', () => {
    ...
    test('every rule carries the plain-English fields the Learning Center renders', () => {
        for (const rule of RULES) {
            for (const field of [
                ...
                assert.ok(rule[field].length > 30, `${rule.key} has a thin "${field}"; a rule nobody can explain is a rule nobody should trust`);
            }
        }
    });
    ...
    test('no rule cites a regulation the filing has not supplied without saying so', () => {
        for (const rule of RULES) {
            if (/CBN|Central Bank of Nigeria|Regulation \d|Section \d/i.test(rule.policyBasis)) {
                assert.match(rule.policyBasis, /UNCONFIRMED|pending the (CBN )?filing/i, `${rule.key} cites a specific regulatory source without marking it unconfirmed`);
            }
        }
    });
    ...
    test('the high-risk jurisdiction list is empty and says why', () => {
        ...
        assert.deepEqual(rule.parameters['jurisdictions'], [], 'the list must be empty: naming jurisdictions would assert an unsupplied regulatory fact');
    ...
    test('prohibited-severity rules reject or suspend, never merely review', () => {
        for (const rule of RULES.filter((r) => r.severity === 'prohibited')) {
            assert.ok(['reject', 'suspend'].includes(rule.onTrigger), `${rule.key} is prohibited-severity but only ${rule.onTrigger}`);
        }
    });
});
// ---------------------------------------------------------------------------
describe('Ledger invariants', () => {
    test('the chart of accounts contains no customer stored-value account', () => {
        ...
        for (const category of categories) {
            assert.ok(!/wallet|balance|client_money|customer_funds|stored_value|custody/.test(category), `"${category}" reads like a custody account; EKORails is not authorised to hold customer funds`);
        }
    ...
    test('every account category has a coherent type and normal side', () => {
        for (const [category, shape] of Object.entries(ACCOUNT_SHAPE)) {
            if (shape.type === 'asset')
                assert.equal(shape.normalSide, 'debit', `${category}`);
            if (shape.type === 'liability')
                assert.equal(shape.normalSide, 'credit', `${category}`);
            if (shape.type === 'income')
                assert.equal(shape.normalSide, 'credit', `${category}`);
        }
    });
    ...
    test('a zero or negative line amount is refused', () => {
        ...
        ]), (e) => e instanceof LedgerError && e.code === 'NON_POSITIVE_AMOUNT');
    });
...
describe('Notification safety', () => {
    test('in-app notifications may carry detail; off-platform ones may not', () => {
        ...
        assert.throws(() => assertSafeForChannel('email', 'Settled', 'Your payment of 4,000,000.00 has settled.'), UnsafeNotificationError);
        assert.throws(() => assertSafeForChannel('email', 'Settled', 'Paid to NL00DEMO0000100001.'), UnsafeNotificationError);
        assert.throws(() => assertSafeForChannel('sms', 'Code', 'Your code is 483920.'), UnsafeNotificationError, 'a one-time code must not be sent in a channel this system does not control');
        assert.throws(() => assertSafeForChannel('email', 'Details', 'Account 0123456789 credited.'), UnsafeNotificationError);
    });
    test('the reference-and-link pattern the product actually uses is permitted', () => {
        assertSafeForChannel('email', 'Transaction update', 'Transaction TXN-REF-ABC has an update. Sign in to view the detail.');
        assertSafeForChannel('sms', 'EKORails', 'You have an update on your EKORails account. Sign in to view it.');
    });
});
