...
import { connect, resetDatabase, buildFixture, expectRejection, createAndAdvance, actor, SYSTEM, } from './helpers.js';
let db;
let fx;
before(async () => {
    resetDatabase();
...
describe('1. Successful B2B settlement', () => {
    test('a transaction runs from creation to completed with a balanced ledger', async () => {
        ...
        for (const j of journals) {
            assert.ok(String(j['plain_english']).length > 50, `journal ${j['reference']} must carry a substantive plain-English explanation`);
        }
    });
...
describe('2. Sanctions match', () => {
    test('a match against the simulated list suspends rather than silently allowing', async () => {
        ...
        assert.ok(['potential_match', 'confirmed_match'].includes(result.status), `expected a match against the fictional list; got ${result.status}`);
        ...
        assert.ok(['suspend', 'reject'].includes(sanctions.action ?? ''), `a sanctions hit must suspend or reject; got ${sanctions.action}`);
    ...
    test('disposing of a match requires a written reason', async () => {
        await db.asOwner(SYSTEM, async (q) => {
            const orgService = await import('../src/modules/org/service.js');
            const row = await q.query("SELECT id FROM screening_case WHERE status IN ('potential_match','confirmed_match') LIMIT 1");
            await expectRejection(orgService.disposeScreening(q, {
...
async function createAndAdvanceInner(q, fixture) {
...
describe('3. PEP escalation', () => {
    test('a PEP hit routes to enhanced due diligence and requires a manager', async () => {
        ...
        const requiresManager = await db.asOwner(SYSTEM, async (q) => {
            const row = await q.query('SELECT requires_manager FROM compliance_case WHERE reference = $1', [evaluation.complianceCaseReference]);
    ...
    test('an analyst cannot approve a high-risk KYB case', async () => {
        await db.asOwner(SYSTEM, async (q) => {
            ...
            await expectRejection(orgService.decideKyb(q, {
...
describe('4. Transaction above limit', () => {
    test('an amount over the per-transaction limit is rejected, not merely flagged', async () => {
        const evaluation = await db.asOwner(SYSTEM, async (q) => {
            const engine = await import('../src/modules/compliance/engine.js');
            const txnService = await import('../src/modules/transaction/service.js');
            const limit = await q.query('SELECT per_transaction_limit::text FROM corridor WHERE id = $1', [fx.corridorId]);
    ...
    test('a missing limit is treated as a block, never as unlimited', async () => {
        const evaluation = await db.asOwner(SYSTEM, async (q) => {
            ...
            await q.query("UPDATE corridor SET per_transaction_limit = '50000000.000000' WHERE id = $1", [fx.corridorId]);
            return result;
        });
        ...
        assert.notEqual(evaluation.recommendedAction, 'auto_continue', 'a transaction with no configured limit must never auto-continue');
    });
});
// ---------------------------------------------------------------------------
describe('5. Missing source-of-funds evidence', () => {
    ...
    test('a short source-of-funds narrative is refused at creation', async () => {
        await db.asOwner(SYSTEM, async (q) => {
            const txnService = await import('../src/modules/transaction/service.js');
            await expectRejection(txnService.createTransaction(q, {
                organizationId: fx.orgAId, beneficiaryId: fx.beneficiaryA, corridorId: fx.corridorId,
...
describe('6. Duplicate invoice', () => {
    test('a second transaction with the same invoice fingerprint is detected', async () => {
        ...
        assert.equal(rule.action, 'manual_review', 'a duplicate must go to review, not auto-reject: instalments and re-issues are legitimate');
        const matches = rule.dataUsed['matches'];
...
describe('7. Expired FX quote', () => {
    test('an expired quote cannot be accepted', async () => {
        await db.asOwner(SYSTEM, async (q) => {
            ...
            await q.query(`UPDATE fx_quote
            SET issued_at = now() - interval '2 hours', expires_at = now() - interval '1 hour'
          WHERE id = $1`, [quote.quoteId]);
            await expectRejection(fxq.acceptQuote(q, {
                quoteId: quote.quoteId, acceptedBy: fx.users.approverA, organizationId: fx.orgAId,
            }), /QUOTE_EXPIRED|expired/i);
            const after = await q.query('SELECT status FROM fx_quote WHERE id = $1', [quote.quoteId]);
    ...
    test('a simulated quote can never be marked as contractually locked', async () => {
        await db.asOwner(SYSTEM, async (q) => {
            const fxq = await import('../src/modules/fx/quotes.js');
            const { Decimal, RATE_SCALE } = await import('../src/core/money.js');
            await expectRejection(fxq.issueQuote(q, {
                organizationId: fx.orgAId, corridorId: fx.corridorId,
                sendAmount: Decimal.fromString('1000000.000000'),
                sendCurrency: 'NGN', receiveCurrency: 'USD',
                referenceRate: Decimal.fromString('0.000625000000', RATE_SCALE),
...
describe('8. Partner API timeout', () => {
    test('a timeout produces an UNKNOWN outcome, a suspense posting and a critical exception', async () => {
        ...
        const detail = await db.asOwner(SYSTEM, async (q) => {
            const journals = await q.query("SELECT journal_type FROM journal WHERE transaction_id = $1", [txn.id]);
            const outbound = await q.query('SELECT state FROM outbound_idempotency WHERE transaction_id = $1', [txn.id]);
            const exceptions = await q.query('SELECT exception_type, priority FROM exception_case WHERE transaction_id = $1', [txn.id]);
            return { journals: journals.rows, outbound: outbound.rows, exceptions: exceptions.rows };
        });
        assert.ok(detail.journals.some((j) => j.journal_type === 'suspense_posting'), 'the instructed amount must be parked in settlement suspense');
        assert.equal(detail.outbound[0]?.state, 'unknown', 'the outbound record must be marked unknown');
        assert.ok(detail.exceptions.some((e) => e.exception_type === 'unknown_partner_outcome' && e.priority === 'critical'), 'a critical unknown-outcome exception must be raised');
    });
});
// ---------------------------------------------------------------------------
describe('9. Duplicate partner callback', () => {
    test('resubmitting the same idempotency key does not instruct a second payment', async () => {
        ...
        const submissions = await db.asOwner(SYSTEM, async (q) => {
            const row = await q.query(`SELECT count(*)::text AS n FROM integration_event
          WHERE transaction_id = $1 AND operation = 'settlement.submit'
            AND outcome NOT IN ('duplicate_ignored')`, [txn.id]);
...
describe('10. Failed settlement', () => {
    test('a partner rejection unwinds the positioning by reversal, not deletion', async () => {
        ...
        const journals = await db.asOwner(SYSTEM, async (q) => {
            const rows = await q.query('SELECT journal_type, posting_status, reference FROM journal WHERE transaction_id = $1 ORDER BY posted_at', [txn.id]);
            return rows.rows;
        });
        ...
        const stillPresent = journals.filter((j) => j.journal_type === 'fx_conversion' || j.journal_type === 'partner_positioning');
...
describe('11. Returned transaction', () => {
    test('a return is a new event; the original settlement journal stands', async () => {
        ...
        const journals = await db.asOwner(SYSTEM, async (q) => {
            const rows = await q.query('SELECT journal_type, posting_status FROM journal WHERE transaction_id = $1', [txn.id]);
            return rows.rows;
        });
        ...
        assert.equal(settlementJournal.posting_status, 'posted', 'the original settlement must NOT be reversed: the payment genuinely happened');
        assert.ok(journals.some((j) => j.journal_type === 'return_receipt'), 'a separate return-receipt journal must record the funds coming back');
    });
});
// ---------------------------------------------------------------------------
describe('12. Reconciliation mismatch', () => {
    test('a partner statement that disagrees with the ledger produces a break', async () => {
        ...
        assert.ok((summary.byResult['amount_difference'] ?? 0) > 0, `expected an amount_difference; got ${JSON.stringify(summary.byResult)}`);
    ...
    test('closing a break above the threshold requires a second person', async () => {
        await db.asOwner(SYSTEM, async (q) => {
            ...
            const row = await q.query(`SELECT id, amount::text FROM exception_case
          WHERE exception_type = 'reconciliation_break' AND status = 'open'
            AND amount >= 1000 ORDER BY opened_at DESC LIMIT 1`);
            if (!row.rows[0])
                return; // no break above the threshold in this run
            const proposed = await exceptions.proposeResolution(q, {
                exceptionId: row.rows[0].id,
                resolution: 'Investigated with the partner. The difference arises from a partner-side rounding ' +
            ...
            await expectRejection(exceptions.approveResolution(q, row.rows[0].id, fx.users.finance), /SEGREGATION_OF_DUTIES/);
            await exceptions.approveResolution(q, row.rows[0].id, fx.users.finance2);
            const after = await q.query('SELECT status FROM exception_case WHERE id = $1', [row.rows[0].id]);
...
describe('13. Unauthorised ledger access', () => {
    ...
    test('the application role cannot UPDATE a journal entry', async () => {
        await expectRejection(db.asApp({ scope: 'global' }, (q) => q.query('UPDATE journal_entry SET amount = 1')), /permission denied|APPEND_ONLY/i);
    });
    test('the application role cannot DELETE a journal entry', async () => {
        await expectRejection(db.asApp({ scope: 'global' }, (q) => q.query('DELETE FROM journal_entry')), /permission denied|APPEND_ONLY/i);
    });
    test('even the schema owner cannot UPDATE a journal entry', async () => {
        await expectRejection(db.asOwner(SYSTEM, (q) => q.query("UPDATE journal_entry SET narrative = 'tampered'")), /APPEND_ONLY_VIOLATION/);
    });
    test('even the schema owner cannot DELETE a journal entry', async () => {
        await expectRejection(db.asOwner(SYSTEM, (q) => q.query('DELETE FROM journal_entry')), /APPEND_ONLY_VIOLATION/);
    });
    test('a business user has no ledger read permission and sees only their own accounts', async () => {
        ...
        assert.equal(businessPermissions.has('ledger.read'), false, 'a business role must not hold ledger.read');
        const visible = await db.asApp({ scope: 'org', organizationId: fx.orgAId }, async (q) => {
            const rows = await q.query(`SELECT count(*)::text AS n FROM ledger_account
          WHERE organization_id IS NOT NULL AND organization_id <> $1`, [fx.orgAId]);
...
describe('14. Unauthorised cross-organisation access', () => {
    test('organisation A cannot see organisation B\'s transactions, at the database level', async () => {
        ...
        const seenByA = await db.asApp({ scope: 'org', organizationId: fx.orgAId }, async (q) => {
            const rows = await q.query('SELECT organization_id FROM transaction');
            return rows.rows;
        });
        assert.ok(seenByA.length > 0, 'organisation A must see its own transactions');
        assert.ok(seenByA.every((r) => r.organization_id === fx.orgAId), 'organisation A must see ONLY its own transactions');
        const seenByB = await db.asApp({ scope: 'org', organizationId: fx.orgBId }, async (q) => {
            const rows = await q.query('SELECT count(*)::text AS n FROM transaction WHERE organization_id = $1', [fx.orgAId]);
            return Number(rows.rows[0].n);
        });
    ...
    test('the isolation holds across every organisation-scoped table', async () => {
        ...
        for (const table of tables) {
            const leaked = await db.asApp({ scope: 'org', organizationId: fx.orgBId }, async (q) => {
                ...
                const rows = await q.query(`SELECT count(*)::text AS n FROM ${table} WHERE ${column} = $1`, [fx.orgAId]);
                return Number(rows.rows[0].n);
            });
    ...
    test('a request with no security context sees nothing at all', async () => {
        const visible = await db.asApp({ scope: 'none' }, async (q) => {
            const rows = await q.query('SELECT count(*)::text AS n FROM transaction');
            return Number(rows.rows[0].n);
        });
    ...
    test('a cross-organisation quote acceptance returns not-found, not forbidden', async () => {
        await db.asOwner(SYSTEM, async (q) => {
            const fxq = await import('../src/modules/fx/quotes.js');
            const row = await q.query('SELECT id FROM fx_quote WHERE organization_id = $1 LIMIT 1', [fx.orgAId]);
            const error = await expectRejection(fxq.acceptQuote(q, {
            ...
            assert.ok(!/forbidden|not yours|other organisation/i.test(error.message), 'the message must not confirm that the record exists elsewhere');
        });
    });
});
// ---------------------------------------------------------------------------
describe('15. Administrator attempting to edit an audit record', () => {
    test('the application role has no UPDATE privilege on the audit trail', async () => {
        await expectRejection(db.asApp({ scope: 'global' }, (q) => q.query("UPDATE audit_event SET action = 'tampered'")), /permission denied/i);
    });
    test('the application role has no DELETE privilege on the audit trail', async () => {
        await expectRejection(db.asApp({ scope: 'global' }, (q) => q.query('DELETE FROM audit_event')), /permission denied/i);
    });
    test('even the schema owner is refused by the append-only trigger on UPDATE', async () => {
        await expectRejection(db.asOwner(SYSTEM, (q) => q.query("UPDATE audit_event SET reason = 'tampered' WHERE seq = 1")), /APPEND_ONLY_VIOLATION/);
    });
    test('even the schema owner is refused by the append-only trigger on DELETE', async () => {
        await expectRejection(db.asOwner(SYSTEM, (q) => q.query('DELETE FROM audit_event WHERE seq = 1')), /APPEND_ONLY_VIOLATION/);
    });
    test('the audit record is genuinely unchanged after the attempts', async () => {
        const row = await db.asOwner(SYSTEM, async (q) => {
            const r = await q.query('SELECT action, reason, entry_hash FROM audit_event WHERE seq = 1');
            return r.rows[0];
        });
    ...
    test('a System Administrator holds no permission that could reach a compliance decision', async () => {
        ...
        for (const forbidden of [
            ...
            assert.equal(admin.has(forbidden), false, `system_administrator must not hold ${forbidden}`);
        }
        assert.ok(ROLES.system_administrator.explicitDenials.some((d) => /audit record/i.test(d)), 'the role must explicitly state it cannot edit audit records');
    });
...
describe('16. Journal imbalance attempt', () => {
    test('an unbalanced journal is refused at commit by the database', async () => {
        await expectRejection(db.asOwner(SYSTEM, async (q) => {
            const accounts = await q.query("SELECT id FROM ledger_account WHERE currency = 'NGN' LIMIT 2");
            const journal = await q.query(`INSERT INTO journal (reference, journal_type, description, plain_english, effective_date)
           VALUES ('JRN-IMBALANCE-TEST','test_liquidity_injection','imbalance',
                   ...
                   CURRENT_DATE)
           RETURNING id`);
            await q.query(`INSERT INTO journal_entry (journal_id, line_number, ledger_account_id, direction, amount, currency, narrative)
           VALUES ($1,1,$2,'debit',1000.00,'NGN','dr'), ($1,2,$3,'credit',999.00,'NGN','cr')`, [journal.rows[0].id, accounts.rows[0].id, accounts.rows[1].id]);
        }), /JOURNAL_IMBALANCE/);
    });
    test('a single-line journal is refused', async () => {
        await expectRejection(db.asOwner(SYSTEM, async (q) => {
            const account = await q.query("SELECT id FROM ledger_account WHERE currency = 'NGN' LIMIT 1");
            const journal = await q.query(`INSERT INTO journal (reference, journal_type, description, plain_english, effective_date)
           ...
           RETURNING id`);
            await q.query(`INSERT INTO journal_entry (journal_id, line_number, ledger_account_id, direction, amount, currency, narrative)
           VALUES ($1,1,$2,'debit',1000.00,'NGN','dr')`, [journal.rows[0].id, account.rows[0].id]);
        }), /JOURNAL_INCOMPLETE/);
    });
    test('the application layer refuses an imbalance before it reaches the database', async () => {
        ...
        ]), (e) => e instanceof LedgerError && e.code === 'JOURNAL_IMBALANCE');
    });
    ...
    test('an entry cannot be posted in a currency other than its account\'s', async () => {
        await expectRejection(db.asOwner(SYSTEM, async (q) => {
           ...
           RETURNING id`);
            await q.query(`INSERT INTO journal_entry (journal_id, line_number, ledger_account_id, direction, amount, currency, narrative)
           VALUES ($1,1,$2,'debit',1000.00,'USD','wrong currency')`, [journal.rows[0].id, account.rows[0].id]);
        }), /CURRENCY_MISMATCH/);
    });
});
// ---------------------------------------------------------------------------
describe('17. Suspended organisation initiating a transaction', () => {
    test('a suspended organisation is refused at creation and the attempt is audited', async () => {
        await db.asOwner(SYSTEM, async (q) => {
            const txnService = await import('../src/modules/transaction/service.js');
            await q.query("UPDATE organization SET suspended_at = now(), suspension_reason = 'Test suspension' WHERE id = $1", [fx.orgBId]);
            await expectRejection(txnService.createTransaction(q, {
            ...
            const audited = await q.query(`SELECT count(*)::text AS n FROM audit_event
          WHERE organization_id = $1 AND outcome = 'denied'
            AND metadata->>'denial_reason' = 'organisation_suspended'`, [fx.orgBId]);
...
describe('18. User trying to approve their own transaction', () => {
    test('the service layer refuses a self-approval and audits the attempt', async () => {
        ...
        await db.asOwner(SYSTEM, async (q) => {
            const txnService = await import('../src/modules/transaction/service.js');
            await txnService.submitForApproval(q, txn.id, actor(fx.users.initiatorA, 'business_initiator', ['txn.initiate']));
            await expectRejection(txnService.businessApprove(q, {
          ...
          WHERE transaction_id = $1 AND outcome = 'denied'
            AND metadata->>'denial_reason' = 'self_approval'`, [txn.id]);
    ...
    test('the database refuses a self-approval even if the service layer is bypassed', async () => {
        ...
        await expectRejection(db.asOwner(SYSTEM, async (q) => {
            await q.query(`INSERT INTO transaction_approval (
             transaction_id, organization_id, approval_type, decision, decided_by, decided_by_role
           ) VALUES ($1,$2,'business_dual_authorisation','approved',$3,'business_approver')`, [txn.id, fx.orgAId, fx.users.initiatorA]);
        }), /SEGREGATION_OF_DUTIES/);
    });
...
describe('19. Personally identifiable information appearing in logs', () => {
    ...
    test('no audit event in the database contains an unredacted secret', async () => {
        const leaks = await db.asOwner(SYSTEM, async (q) => {
            ...
            const rows = await q.query(`SELECT seq::text,
           ...
           FROM audit_event`);
            ...
            for (const row of rows.rows) {
                ...
                if (l.length > 0)
                    found.push({ seq: row.seq, leaks: l });
            }
            return found;
        });
    ...
    test('no integration event payload contains an unmasked account identifier', async () => {
        const leaks = await db.asOwner(SYSTEM, async (q) => {
            const { findLeaks } = await import('../src/core/redact.js');
            const rows = await q.query(`SELECT id::text,
    ...
    test('off-platform notifications refuse to carry financial detail', async () => {
        ...
        assert.throws(() => assertSafeForChannel('email', 'Payment settled', 'Your payment of NGN 4,000,000.00 has settled.'), (e) => e instanceof UnsafeNotificationError, 'an email must not carry a monetary amount');
        assert.throws(() => assertSafeForChannel('sms', 'Code', 'Your account 0123456789 was credited.'), (e) => e instanceof UnsafeNotificationError, 'an SMS must not carry an account identifier');
        // The pattern actually used by the product: a reference and a prompt to sign in.
        assertSafeForChannel('email', 'Transaction update', 'Transaction TXN-REF has an update. Sign in to view the detail.');
    });
    test('no stored notification body violates the off-platform rule', async () => {
        const violations = await db.asOwner(SYSTEM, async (q) => {
            ...
            const rows = await q.query("SELECT id::text, channel, subject, body FROM notification WHERE channel <> 'in_app'");
            const bad = [];
            for (const row of rows.rows) {
                try {
                    assertSafeForChannel(row.channel, row.subject, row.body);
                }
                catch {
...
describe('20. Backup restoration and transaction-history verification', () => {
    test('a logical backup restores with history, ledger balance and audit chain intact', async () => {
        ...
        const before = await db.asOwner(SYSTEM, async (q) => {
            ...
            const { verifyAuditChain } = await import('../src/audit/audit.js');
            const txns = await q.query('SELECT reference, state FROM transaction ORDER BY reference');
            return {
        ...
        assert.ok(ownerDumpFailed, 'a dump run as the schema owner is expected to fail under FORCE ROW LEVEL SECURITY; ' +
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    const restoredTxns = await client.query('SELECT reference, state FROM transaction ORDER BY reference');
                    assert.deepEqual(restoredTxns.rows, before.transactions, 'every transaction and its state must survive the restore');
                    const trial = await client.query('SELECT currency, difference::text FROM trial_balance ORDER BY currency');
                    for (const row of trial.rows) {
                        assert.equal(Number(row.difference), 0, `the restored ledger must balance in ${row.currency}`);
                    }
                    const chain = await client.query('SELECT count(*)::text AS n FROM verify_audit_chain()');
                    assert.equal(Number(chain.rows[0].n), 0, 'the audit hash chain must still verify after restoration');
                    const journals = await client.query('SELECT count(*)::text AS n FROM journal_entry');
                    assert.equal(Number(journals.rows[0].n), before.ledger.entryCount, 'every journal entry must survive the restore');
                    ...
                    try {
                        ...
                        guardHeld = /APPEND_ONLY_VIOLATION/.test(error instanceof Error ? error.message : String(error));
                    }
...
//# sourceMappingURL=mandatory.test.js.map