...
import { resetDatabase, connect, buildFixture, SYSTEM } from './helpers.js';
...
before(async () => {
    ...
    server = createHttpServer({
        ...
        verifyCsrf: (token, csrf) => withReadOnlyContext({ scope: 'system' }, (q) => auth.verifyCsrf(q, token, csrf)),
    });
...
class Client {
     ...
     */
    forwardedFor = `198.51.100.${(clientCounter += 1) % 250}`;
    ...
    async postWithoutCsrf(path, body = {}) {
        const headers = {
    ...
    address() { return this.forwardedFor; }
}
async function currentTotp(email) {
    const secret = await db.asOwner(SYSTEM, async (q) => {
        const row = await q.query('SELECT mfa_secret_encrypted FROM app_user WHERE email_normalised = $1', [email.toLowerCase()]);
        return row.rows[0]?.mfa_secret_encrypted ?? null;
    });
    if (!secret)
        throw new Error(`${email} has no MFA secret`);
...
describe('Public endpoints and security headers', () => {
    test('the environment banner is on every response, including errors', async () => {
        ...
        assert.match(notFound.headers.get('x-ekorails-environment') ?? '', /NO LIVE FUNDS/, 'even a 404 must carry the banner');
    });
    ...
    test('the regulatory boundary is served publicly and states what EKORails is not', async () => {
        ...
        const isNot = data['ekorails_is_not'];
        for (const claim of [
    ...
    test('OpenAPI describes every route with its required permissions', async () => {
        ...
        assert.match(String(spec['info'] && spec['info']['description']), /not a bank/);
    });
});
// ---------------------------------------------------------------------------
describe('Authentication', () => {
    test('a wrong password and an unknown email give the same response', async () => {
        ...
        assert.deepEqual(wrongPassword.json['error'], unknownEmail.json['error'], 'the endpoint must not be a user-enumeration oracle');
    });
    test('a successful login sets an httpOnly session cookie and a readable CSRF cookie', async () => {
        ...
        assert.ok(!JSON.stringify(response.json).includes(session.split('=')[1].split(';')[0]), 'the session token must not be echoed in the body');
    });
    test('failed logins lock the account after the threshold', async () => {
        ...
        const status = await db.asOwner(SYSTEM, async (q) => {
            const row = await q.query('SELECT status, locked_until FROM app_user WHERE email_normalised = $1', [email]);
            return row.rows[0];
        });
    ...
    test('every login attempt, successful or not, is recorded', async () => {
        const count = await db.asOwner(SYSTEM, async (q) => {
            const row = await q.query(`SELECT count(*) FILTER (WHERE succeeded)::text AS succeeded,
    ...
    test('an unauthenticated request to a protected route is refused', async () => {
        ...
        assert.equal(response.status, 401);
        assert.equal(response.json['error']['code'], 'SESSION_REQUIRED');
    });
...
describe('CSRF protection', () => {
    test('a state-changing request without the CSRF header is refused', async () => {
        ...
        assert.equal(response.json['error']['code'], 'CSRF_TOKEN_INVALID');
    });
    test('a request with the WRONG CSRF token is refused', async () => {
        const client = await signIn('initiator.a@testalpha.invalid');
        const response = await client.post('/api/support-cases', { category: 'customer_support', subject: 'Test', description: 'Test description.' }, { 'x-csrf-token': 'not-the-right-token' });
        assert.equal(response.status, 403);
    });
...
describe('Authorisation at the route boundary', () => {
    ...
    test('a compliance analyst cannot route a settlement', async () => {
        const client = await signIn('analyst@ekorails.invalid');
        const response = await client.post('/api/transactions/00000000-0000-0000-0000-000000000000/settlement/submit');
        assert.equal(response.status, 403);
    });
    test('a treasury operator cannot clear a compliance alert', async () => {
        const client = await signIn('treasury@ekorails.invalid');
        const response = await client.post('/api/transactions/00000000-0000-0000-0000-000000000000/compliance-decision', { decision: 'approve', reason: 'Attempting to clear an alert as treasury, which must be refused.' });
        assert.equal(response.status, 403);
    });
    ...
    test('/api/me reports the caller\'s roles, permissions and explicit denials', async () => {
        ...
        const roleDetails = data['role_details'];
...
describe('Cross-organisation isolation over HTTP', () => {
    test('a user sees only their own organisation\'s transactions', async () => {
        ...
        const alphaList = (await alpha.get('/api/transactions')).json['data'];
        const bravoList = (await bravo.get('/api/transactions')).json['data'];
    ...
    test('fetching another organisation\'s transaction by id returns 404', async () => {
        const alpha = await signIn('initiator.a@testalpha.invalid');
        const bravo = await signIn('initiator.b@testbravo.invalid');
        const list = (await alpha.get('/api/transactions')).json['data'];
        ...
        assert.ok(!JSON.stringify(denied.json).toLowerCase().includes('forbidden'), 'the response must not hint that the record exists elsewhere');
    });
});
// ---------------------------------------------------------------------------
...
describe('Input validation', () => {
    ...
    test('a missing required field names the field', async () => {
        ...
        assert.ok(error['details']['field'], 'the failing field must be named');
    });
...
describe('Reporting over HTTP', () => {
    test('the report catalogue is filtered by the caller\'s permissions', async () => {
        ...
        const businessReports = (await business.get('/api/reports')).json['data'];
        const financeReports = (await finance.get('/api/reports')).json['data'];
        assert.ok(!businessReports.some((r) => r.key === 'trial-balance'), 'a business user must not be offered the trial balance');
        assert.ok(financeReports.some((r) => r.key === 'trial-balance'), 'a finance analyst must be offered the trial balance');
    });
    test('a report exports as CSV, XLSX and PDF, and each export is recorded', async () => {
        ...
        for (const [format, expectedType] of [
            ...
            assert.match(String(response.headers.get('content-type') ?? ''), new RegExp(expectedType), `${format} must be served with the right content type`);
            assert.match(String(response.headers.get('content-disposition') ?? ''), /attachment; filename=/, `${format} must be served as a download`);
        }
        const recorded = await db.asOwner(SYSTEM, async (q) => {
            const rows = await q.query("SELECT format, content_sha256, masking_profile FROM report WHERE report_key = 'trial-balance'");
            return rows.rows;
        });
        ...
        const audited = await db.asOwner(SYSTEM, async (q) => {
            const row = await q.query("SELECT count(*)::text AS n FROM audit_event WHERE category = 'report_export'");
...
describe('Founder Learning Center over HTTP', () => {
    test('the product map reports honest completion stages', async () => {
        ...
        assert.equal(response.status, 200);
        const data = response.json['data'];
        const definitions = data['completion_definitions'];
    ...
    test('the state-machine view disclaims settlement finality', async () => {
        ...
        assert.ok(data['transition_count'] > 25);
    });
    test('the compliance rule library is readable with plain-English explanations', async () => {
        ...
        for (const rule of rules) {
            assert.ok(String(rule['risk_addressed'] ?? '').length > 30, `${rule['rule_key']} needs a real explanation of the risk it addresses`);
            assert.ok(String(rule['false_positive_risk'] ?? '').length > 20, `${rule['rule_key']} must state how it can be wrong`);
        }
    });
});
