...
import { seedRolesAndPermissions, seedCorridor, seedPartners, seedRules, seedConfiguration, seedFeatureFlags, seedRetentionPolicies, seedApprovalMatrix, seedRequiredDocuments, } from './reference.js';
import { seedLearningContent, seedGlossary, seedDecisionLog, seedRiskRegister, seedBuildJournal, } from './learning.js';
import { seedInternalUsers, seedBusinesses, seedBeneficiaries, seedLiquidity, seedTransactions, seedIncidentAndComplaint, seedSanctionsFalsePositive, seedPepEscalation, DEMO_PASSWORD, DEMO_NOTICE, } from './demo.js';
...
async function main() {
    ...
    if (env.mode !== 'DEMO' && env.mode !== 'SANDBOX') {
        throw new Error(`REFUSED: seeding creates users with a known password and fictional customer data. ` +
            `It will not run with EKORAILS_ENV_MODE=${env.mode}.`);
    }
    logger.info('Seeding started', { mode: env.mode, notice: DEMO_NOTICE });
    const summary = { environment: env.mode };
    // ---- Reference data -----------------------------------------------------
    ...
    const internalOrgId = await withContext({ scope: 'system' }, async (db) => {
        const existing = await db.query("SELECT id FROM organization WHERE display_code = 'ORG-EKORAILS'");
        if (existing.rows[0])
            return existing.rows[0].id;
        const row = await one(db, `INSERT INTO organization (display_code, legal_name, kind, onboarding_status)
    ...
    const orgs = await withContext({ scope: 'system' }, (db) => seedBusinesses({ db, ...ctxBase }));
    summary['organizations'] = orgs.size;
    const beneficiaries = await withContext({ scope: 'system' }, (db) => seedBeneficiaries({ db, ...ctxBase }, orgs));
    summary['beneficiaries'] = beneficiaries.size;
    await withContext({ scope: 'system' }, (db) => seedLiquidity({ db, ...ctxBase }));
    await withContext({ scope: 'system' }, (db) => seedSanctionsFalsePositive({ db, ...ctxBase }));
    await withContext({ scope: 'system' }, (db) => seedPepEscalation({ db, ...ctxBase }));
    const txns = await withContext({ scope: 'system' }, (db) => seedTransactions({ db, ...ctxBase }, orgs, beneficiaries));
    summary['transactions'] = txns.created;
    summary['transactions_by_outcome'] = txns.byOutcome;
    await withContext({ scope: 'system' }, (db) => seedIncidentAndComplaint({ db, ...ctxBase }));
    // ---- Reconciliation and delivery ---------------------------------------
    const recon = await withContext({ scope: 'system' }, (db) => runDailyReconciliation(db, new Date(), users['finance']));
    ...
    const accounts = await withContext({ scope: 'system' }, (db) => db.query(`SELECT u.email, u.full_name,
    ...
    for (const a of accounts.rows) {
        process.stdout.write(`  ${a.email.padEnd(42)} ${(a.roles ?? []).join(', ').padEnd(38)} ${a.org}\n`);
    }
