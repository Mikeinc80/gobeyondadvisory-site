...
async function main() {
    ...
    const row = await withReadOnlyContext({ scope: 'system' }, (db) => maybeOne(db, 'SELECT full_name, mfa_secret_encrypted, mfa_enrolled FROM app_user WHERE email_normalised = $1', [email.trim().toLowerCase()]));
    if (!row) {
