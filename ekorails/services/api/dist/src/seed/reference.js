...
export async function seedRolesAndPermissions(db) {
    for (const [code, permission] of Object.entries(PERMISSIONS)) {
        await db.query(`INSERT INTO permission (code, description, domain, is_sensitive)
       VALUES ($1,$2,$3,$4)
       ON CONFLICT (code) DO UPDATE SET
         description = EXCLUDED.description, domain = EXCLUDED.domain,
         is_sensitive = EXCLUDED.is_sensitive`, [code, permission.description, permission.domain, permission.sensitive]);
    }
    for (const role of Object.values(ROLES)) {
        await db.query(`INSERT INTO role (code, name, description, realm, requires_step_up, is_break_glass)
       VALUES ($1,$2,$3,$4,$5,$6)
       ON CONFLICT (code) DO UPDATE SET
         name = EXCLUDED.name, description = EXCLUDED.description, realm = EXCLUDED.realm,
         requires_step_up = EXCLUDED.requires_step_up, is_break_glass = EXCLUDED.is_break_glass`, [role.code, role.name, role.description, role.realm, role.requiresStepUp, role.isBreakGlass]);
        for (const permission of role.permissions) {
            await db.query('INSERT INTO role_permission (role_code, permission_code) VALUES ($1,$2) ON CONFLICT DO NOTHING', [role.code, permission]);
        }
    }
}
export async function seedCorridor(db) {
    const existing = await db.query("SELECT id FROM corridor WHERE code = 'PILOT-CORRIDOR-1'");
    if (existing.rows[0])
        return existing.rows[0].id;
    const row = await one(db, `INSERT INTO corridor (
...
export async function seedPartners(db) {
    const definitions = [
        {
            key: 'originBank', code: 'SIM-ORIGIN-BANK', name: 'Simulated Origin Bank',
            role: 'origin_bank',
            liveResponsibility: 'Receives and holds the customer\'s funds in the origin currency, and confirms receipt to ' +
                'EKORails. EKORails never touches these funds.',
            licensedActivity: 'Deposit-taking and payment services in the origin jurisdiction. EKORails performs none of ' +
        ...
        },
        {
            ...
            liveResponsibility: 'Quotes and executes the currency conversion, and bears the market risk between quote and ' +
                'execution where a rate is contractually locked.',
            licensedActivity: 'Foreign exchange dealing. EKORails does not deal in foreign exchange on its own account.',
            adapterKey: 'simulated_settlement_v1',
        },
        {
            key: 'settlementInstitution', code: 'SIM-SETTLEMENT', name: 'Simulated Settlement Institution',
            role: 'settlement_institution',
            liveResponsibility: 'Executes the payment to the beneficiary\'s bank and reports the outcome. Settlement ' +
                'finality, where it exists, is conferred here — not by EKORails.',
            licensedActivity: 'Payment execution and access to a settlement system. EKORails has no such access.',
            adapterKey: 'simulated_settlement_v1',
        },
        {
            key: 'destinationBank', code: 'SIM-DEST-BANK', name: 'Simulated Destination Bank',
            role: 'destination_bank',
            liveResponsibility: 'Credits the beneficiary\'s account, applies its own charges, and returns the payment where ' +
        ...
        },
        {
            ...
            liveResponsibility: 'Screens parties against sanctions lists, PEP registers and adverse media, and returns ' +
    ...
    for (const d of definitions) {
        const row = await one(db, `INSERT INTO partner (
       ...
       RETURNING id`, [d.code, d.name, d.role, d.liveResponsibility, d.licensedActivity, d.adapterKey]);
        result[d.key] = row.id;
        for (const [label, currency, purpose] of [
            ['operating', DEMO_SEND_CURRENCY, 'funding'],
            ['operating', DEMO_RECEIVE_CURRENCY, 'settlement'],
        ]) {
            await db.query(`INSERT INTO partner_account (partner_id, account_label, currency, purpose, is_simulated)
         VALUES ($1,$2,$3,$4,true) ON CONFLICT DO NOTHING`, [row.id, label, currency, purpose]);
        }
    }
    return result;
}
 ...
 */
export async function seedRules(db, proposerId, approverId) {
    let count = 0;
    for (const rule of RULES) {
        const existing = await db.query('SELECT 1 FROM risk_rule WHERE rule_key = $1 AND version = $2', [rule.key, rule.version]);
        if (existing.rows.length > 0)
            continue;
        await db.query(`INSERT INTO risk_rule (
...
export async function seedConfiguration(db, proposerId, approverId) {
    const entries = [
        {
            key: 'sandbox_admission_status', value: 'not_confirmed', type: 'string',
            description: 'Whether EKORails has been admitted to the CBN Regulatory Sandbox. Defaults to not_confirmed. ' +
        ...
        },
        {
            ...
            description: 'Only the registered name is asserted. Jurisdiction of incorporation, company number and ' +
        ...
        },
        {
            ...
            description: 'The real settlement mechanism (correspondent banking, licensed PSP or other) is not ' +
        ...
        },
        {
            ...
            description: 'The approved deployment region. The system does NOT claim African data residency; residency ' +
        ...
        },
        {
            ...
            description: 'Nigerian AML/CFT reporting thresholds are UNCONFIRMED. The rule set implements generally ' +
        ...
        },
        {
            ...
            description: 'How often an approved customer\'s KYB is re-reviewed. A conservative default; the ' +
    ...
    for (const entry of entries) {
        const existing = await db.query('SELECT 1 FROM system_configuration WHERE config_key = $1 AND is_current', [entry.key]);
        if (existing.rows.length > 0)
            continue;
        await db.query(`INSERT INTO system_configuration (
...
export async function seedFeatureFlags(db) {
    const flags = [
        {
            key: 'live_money_movement',
            description: 'Master switch for real money movement. Immutable at runtime: it is governed by the process ' +
        ...
        },
        {
            ...
            description: 'AI-assisted extraction of invoice fields. Proposals only; nothing takes effect without ' +
                'human confirmation.',
            releaseGate: false, immutable: false,
        },
        {
    ...
    for (const flag of flags) {
        const result = await db.query(`INSERT INTO feature_flag (key, description, enabled, is_release_gate, is_immutable, change_reason)
...
export async function seedRetentionPolicies(db) {
    const policies = [
        {
            category: 'kyb_records',
            description: 'Customer due diligence records, ownership registers and supporting documents.',
            months: 84,
            basis: 'AML record-keeping obligations require customer due diligence records to be retained for a ' +
            ...
            erasurePermitted: false, disposal: 'archive',
        },
        {
        ...
        },
        {
            ...
            months: 84,
            basis: 'Retained to evidence that screening took place and what it returned. Contains personal data, ' +
                'so it is classified restricted and access is separately audited.',
            erasurePermitted: false, disposal: 'delete',
        },
        {
            ...
            erasurePermitted: true, disposal: 'delete',
        },
        {
            ...
            erasurePermitted: false, disposal: 'anonymise',
        },
        {
    ...
    for (const p of policies) {
        const result = await db.query(`INSERT INTO retention_policy (
         data_category, description, retention_months, legal_basis, erasure_permitted, disposal_method
       ) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (data_category) DO NOTHING`, [p.category, p.description, p.months, p.basis, p.erasurePermitted, p.disposal]);
        count += result.rowCount ?? 0;
    }
    return count;
}
export async function seedApprovalMatrix(db) {
    const entries = [
        {
    ...
    for (const e of entries) {
        const result = await db.query(`INSERT INTO approval_matrix (
...
export async function seedRequiredDocuments(db, corridorId) {
    const rules = [
    ...
    for (const r of rules) {
        const existing = await db.query('SELECT 1 FROM required_document_rule WHERE applies_to = $1 AND document_type = $2', [r.appliesTo, r.type]);
        if (existing.rows.length > 0)
            continue;
        await db.query(`INSERT INTO required_document_rule (
         applies_to, corridor_id, document_type, is_mandatory, validity_months, status
       ) VALUES ($1,$2,$3,$4,$5,'active')`, [r.appliesTo, r.appliesTo === 'transaction' ? corridorId : null, r.type, r.mandatory, r.validity]);
        count += 1;
    }
    return count;
}
