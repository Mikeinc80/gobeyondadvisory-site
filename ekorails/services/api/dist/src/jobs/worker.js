...
async function expireQuotes() {
    return withContext({ scope: 'system' }, async (db) => {
        ...
        if (expired > 0) {
            ...
            const stranded = await many(db, `SELECT t.id, t.reference, t.organization_id
...
async function warnExpiringQuotes() {
    return withContext({ scope: 'system' }, async (db) => {
        const soon = await many(db, `SELECT q.id, q.reference, q.organization_id, q.issued_by AS accepted_by
         FROM fx_quote q
...
async function flagExpiringDocuments() {
    return withContext({ scope: 'system' }, async (db) => {
        const docs = await many(db, `SELECT id, organization_id, document_type, expires_on
         FROM document
...
async function markPeriodicReviews() {
    return withContext({ scope: 'system' }, async (db) => {
        ...
        const { rowCount } = await db.query(`UPDATE organization o
...
async function ageServiceLevels() {
    return withContext({ scope: 'system' }, async (db) => {
        const compliance = await one(db, `SELECT count(*)::text AS n FROM compliance_case
        WHERE sla_due_at < now() AND status NOT LIKE 'closed%'`);
        const exceptionCases = await one(db, `SELECT count(*)::text AS n FROM exception_case
...
async function pruneExpiredState() {
    return withContext({ scope: 'system' }, async (db) => {
        ...
        const sessions = await db.query("DELETE FROM user_session WHERE absolute_expiry < now() - interval '7 days'");
...
export function startWorker() {
    ...
    const tick = async () => {
        ...
        try {
            ...
            const interesting = Object.entries(result).some(([k, v]) => k !== 'duration_ms' && typeof v === 'number' && v > 0);
            if (interesting)
