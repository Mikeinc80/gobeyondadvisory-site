...
export const SYSTEM_CONTEXT = { scope: 'system' };
export const ANONYMOUS_CONTEXT = { scope: 'none' };
let pool = null;
...
export function useDeploymentCredentials() {
    ...
    if (!['DEMO', 'SANDBOX', 'TEST'].includes(mode)) {
        throw new Error(`REFUSED: deployment credentials cannot be used with EKORAILS_ENV_MODE=${mode}. ` +
 ...
 */
export async function withReadOnlyContext(context, fn) {
    const client = await getPool().connect();
    try {
        ...
        catch { /* connection is being discarded */ }
        throw error;
    }
    finally {
        client.release();
...
export async function one(db, text, params = []) {
    ...
    if (rows.length !== 1) {
        throw internal('UNEXPECTED_ROW_COUNT', `Expected exactly 1 row, received ${rows.length}. Query: ${text.slice(0, 200)}`);
    }
    return rows[0];
}
/** At most one row. Returns null where a record legitimately may not exist. */
export async function maybeOne(db, text, params = []) {
    const { rows } = await db.query(text, params);
    if (rows.length > 1) {
...
export function mapDatabaseError(error) {
    if (!(error instanceof Error))
        return internal('DB_ERROR', String(error));
    ...
    if (pgError.code === '23505') {
        const dup = new Error('A record with these details already exists.');
        dup.name = 'UniqueViolation';
        dup.code = 'UNIQUE_VIOLATION';
        return dup;
    }
    if (pgError.code === '42501') {
        const denied = new Error('The application database role is not permitted to perform this operation.');
...
export async function advisoryLock(db, namespace, key) {
