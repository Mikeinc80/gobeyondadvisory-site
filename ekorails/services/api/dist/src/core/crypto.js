 ...
 */
import { createCipheriv, createDecipheriv, createHash, createHmac, randomBytes, scryptSync, timingSafeEqual, hkdfSync, } from 'node:crypto';
...
function masterKey() {
    ...
    if (mode === 'PRODUCTION' || mode === 'CONTROLLED_PILOT') {
        throw new Error('EKORAILS_MASTER_KEY is not set. Refusing to fall back to a development key outside ' +
...
function fingerprintKey() {
    return Buffer.from(hkdfSync('sha256', masterKey(), Buffer.alloc(0), Buffer.from('ekorails/fingerprint'), 32));
}
function dataKey(keyId) {
    return Buffer.from(hkdfSync('sha256', masterKey(), Buffer.alloc(0), Buffer.from(`ekorails/field/${keyId}`), 32));
}
/**
 ...
 */
export function fingerprint(value, domain) {
...
export function verifyPassword(password, stored) {
    ...
    if (parts.length !== 3 || parts[0] !== PASSWORD_ALGO)
        return false;
...
export function checkPasswordPolicy(password, context = {}) {
    ...
    if (password.length > 256)
        failures.push('Must be at most 256 characters long.');
...
export function encryptField(plaintext, keyId = CURRENT_KEY_ID) {
...
export function totpCodeForStep(secretBase32, step) {
 ...
 */
export function verifyTotp(secretBase32, code, lastUsedStep, atMs = Date.now()) {
    const normalised = code.replace(/\D/g, '');
    if (normalised.length !== TOTP_DIGITS)
...
export function totpProvisioningUri(secretBase32, account, issuer = 'EKORails') {
...
export function signStorageUrl(storageKey, expiresAtMs) {
...
export function verifyStorageUrl(storageKey, expiresAtMs, signature) {
