...
function parseMode(raw) {
    ...
    if (value === 'TEST')
        return 'DEMO';
    throw new EnvironmentError(`Unknown EKORAILS_ENV_MODE "${raw}". Valid values: DEMO, SANDBOX, CONTROLLED_PILOT, PRODUCTION.`);
}
function gateIsMet(key, source) {
...
export function describeEnvironment(source = process.env) {
    ...
    if (mode === 'PRODUCTION' && unmetGates.length > 0) {
        throw new EnvironmentError(`Refusing to start in PRODUCTION: ${unmetGates.length} release gate(s) are not met.\n` +
    ...
    if (mode === 'CONTROLLED_PILOT') {
        ...
        if (source['EKORAILS_PILOT_AUTHORISATION_REF'] === undefined) {
            throw new EnvironmentError('Refusing to start in CONTROLLED_PILOT without EKORAILS_PILOT_AUTHORISATION_REF. ' +
 ...
 * through it.
 */
export function assertLiveMoneyPermitted(operation) {
    const env = environment();
    if (!env.liveFundsEnabled) {
        throw new EnvironmentError(`LIVE_FUNDS_DISABLED: "${operation}" would move real money. ` +
...
export function environmentSummary() {
    ...
    return {
        ...
        note: 'The environment mode is process configuration. It cannot be changed through the API, ' +
