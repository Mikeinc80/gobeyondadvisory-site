 ...
 */
export async function nextReference(db, kind, now = new Date()) {
    const { rows } = await db.query('SELECT nextval($1) AS n', [SEQUENCES[kind]]);
    const n = rows[0].n;
 ...
 */
export function outboundIdempotencyKey(transactionReference, operation) {
