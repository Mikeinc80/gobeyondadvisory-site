...
export async function createTransaction(db, input) {
    ...
    if (input.sourceOfFunds.trim().length < 20) {
        throw invalid('SOURCE_OF_FUNDS_REQUIRED', 'Describe the source of funds in at least 20 characters. This is a due-diligence requirement, ' +
            'not a formality.');
    }
    // Idempotency: a repeated create with the same key returns the original.
    if (input.idempotencyKey) {
        const existing = await maybeOne(db, 'SELECT id, reference, state FROM transaction WHERE organization_id = $1 AND idempotency_key = $2', [input.organizationId, input.idempotencyKey]);
        if (existing) {
    ...
    const org = await one(db, 'SELECT onboarding_status, suspended_at, legal_name FROM organization WHERE id = $1', [input.organizationId]);
    if (org.suspended_at !== null) {
        ...
        throw forbidden('ORGANISATION_SUSPENDED', 'This organisation is suspended and cannot initiate transactions.', 'suspended_at is set');
    }
    if (org.onboarding_status !== 'approved') {
        ...
        throw forbidden('ORGANISATION_NOT_APPROVED', `This organisation is "${org.onboarding_status}" and cannot initiate transactions until onboarding is approved.`);
    }
    const beneficiary = await maybeOne(db, 'SELECT id, organization_id, status FROM beneficiary WHERE id = $1', [input.beneficiaryId]);
    ...
    let created;
    try {
        created = await one(db, `INSERT INTO transaction (
       ...
       RETURNING id, state`, [
        ...
        if (pgError.code === '23505') {
            throw conflict('DUPLICATE_IDEMPOTENCY_KEY', 'A transaction with this idempotency key already exists for your organisation.');
        }
        throw error;
    }
    for (const link of input.documentLinks ?? []) {
        const doc = await maybeOne(db, 'SELECT organization_id, malware_scan_status FROM document WHERE id = $1', [link.documentId]);
        ...
        if (doc.malware_scan_status !== 'clean' && doc.malware_scan_status !== 'skipped_unsupported_type') {
            throw precondition('DOCUMENT_NOT_SCANNED', `Document ${link.documentId} has scan status "${doc.malware_scan_status}" and cannot be linked ` +
                `to a transaction until it is clean.`);
        }
        await db.query(`INSERT INTO transaction_document (transaction_id, document_id, role, linked_by)
       VALUES ($1,$2,$3,$4) ON CONFLICT DO NOTHING`, [created.id, link.documentId, link.role, input.initiatedBy]);
    }
    await recordAudit(db, {
...
export async function submitForApproval(db, transactionId, actor) {
    const txn = await one(db, 'SELECT organization_id, reference, initiated_by FROM transaction WHERE id = $1', [transactionId]);
 ...
 */
export async function businessApprove(db, input) {
    const txn = await one(db, 'SELECT organization_id, reference, initiated_by, state FROM transaction WHERE id = $1', [input.transactionId]);
    if (input.actor.userId === txn.initiated_by) {
        ...
        throw forbidden('SEGREGATION_OF_DUTIES', 'You initiated this transaction and cannot also authorise it. A second person must approve.', 'maker-checker on business_dual_authorisation');
    }
    if (!input.approve) {
        await db.query(`INSERT INTO transaction_approval (
    ...
    await db.query('UPDATE transaction SET approved_by = $2, approved_at = now() WHERE id = $1', [input.transactionId, input.actor.userId]);
    const result = await transition(db, {
        transactionId: input.transactionId,
    ...
    const assessment = await evaluate(db, complianceInput, { type: 'transaction', id: input.transactionId, organizationId: txn.organization_id }, { userId: null, role: null });
    await db.query('UPDATE transaction SET risk_rating = $2, latest_risk_assessment_id = $3 WHERE id = $1', [input.transactionId, assessment.outcome, assessment.riskAssessmentId]);
    if (assessment.recommendedAction !== 'auto_continue') {
        await queueNotification(db, {
            ...
            body: `Transaction ${txn.reference} raised ${assessment.triggered.length} rule(s) and needs review. ` +
...
export async function complianceDecide(db, input) {
    if (input.reason.trim().length < 20) {
        throw invalid('REASON_TOO_SHORT', 'A compliance decision requires a written reason of at least 20 characters.');
    }
    const txn = await one(db, 'SELECT organization_id, reference, initiated_by FROM transaction WHERE id = $1', [input.transactionId]);
    const EVENT = {
...
export async function listTransactions(db, filter = {}) {
    return many(db, `SELECT t.id, t.reference, t.state, t.send_amount::text AS send_amount, t.send_currency,
...
export async function getTransaction(db, transactionId) {
    return maybeOne(db, `SELECT t.*, t.send_amount::text AS send_amount,
...
export async function requiringAction(db, organizationId, userId, permissions) {
    const states = [];
    if (permissions.has('txn.approve'))
        states.push('pending_business_approval');
    if (permissions.has('txn.initiate'))
        states.push('draft', 'additional_information_required');
    if (permissions.has('fx.quote.accept'))
        states.push('quote_issued');
    if (states.length === 0)
        return [];
    return many(db, `SELECT t.id, t.reference, t.state, t.send_amount::text AS send_amount, t.send_currency,
      ...
      ORDER BY t.created_at DESC LIMIT 50`, [organizationId, states, userId]);
}
