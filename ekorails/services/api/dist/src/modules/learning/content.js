 ...
 */
import { many, maybeOne } from '../../db/pool.js';
...
export async function productMap(db) {
    const modules = await many(db, 'SELECT * FROM learning_module ORDER BY ordinal');
    return {
        modules,
        ...
        honesty_note: 'A feature is never reported as complete because its interface exists. The stage shown for each ' +
            'module is the highest stage it has genuinely reached.',
    };
}
export async function glossary(db) {
    return many(db, 'SELECT * FROM learning_glossary ORDER BY term');
}
        ...
        components: [
            {
                ...
                layer: 'frontend',
                plain_english: 'The screens people use. Six separate consoles — business, operations, compliance, finance, ' +
                ...
                talks_to: ['api'],
                security_note: 'Runs under a strict Content-Security-Policy with no inline scripts and no external origins. ' +
            ...
            },
            {
                ...
                layer: 'backend',
                plain_english: 'Every rule in the product lives here. It authenticates the caller, checks their permission ' +
                ...
                security_note: 'Connects to the database as a restricted role with no UPDATE or DELETE privilege on audit, ' +
            ...
            },
            {
                ...
                name: 'PostgreSQL',
                layer: 'data',
                plain_english: 'Where everything is stored, and — more importantly — where the rules that must never be ' +
                ...
                talks_to: [],
                security_note: 'Row-level security with FORCE on every table carrying customer data, plus append-only ' +
            ...
            },
            {
                ...
                layer: 'data',
                plain_english: 'The financial record. Every movement is two or more entries that sum to zero within each ' +
                    'currency. Balances are never stored — they are always added up from the entries, so there ' +
                    'is nothing to drift out of line.',
                talks_to: ['database'],
                security_note: 'Corrections are made by posting a reversal, never by changing an entry. Both the mistake ' +
            ...
            },
            {
                ...
                layer: 'backend',
                plain_english: 'Runs every rule against every transaction and records what it found — including the rules ' +
                    'that did not fire. Screening for sanctions, PEP and adverse media goes out to a provider.',
                talks_to: ['database', 'partners'],
                security_note: 'Evaluations are immutable and self-contained: they store the rule text and parameter values ' +
            ...
            },
            {
                ...
                layer: 'external',
                plain_english: 'The licensed institutions that actually hold and move money: the origin bank, the FX ' +
                    'provider, the settlement institution and the destination bank. EKORails instructs them; ' +
                    'it does not do their job.',
                talks_to: [],
                security_note: 'Every instruction carries an idempotency key derived from the transaction reference, so the ' +
            ...
            },
            {
                ...
                layer: 'data',
                plain_english: 'Where uploaded invoices, contracts and identity documents live. Downloads go through a ' +
                    'short-lived signed link, and every link that is minted is recorded.',
                talks_to: [],
                security_note: 'Encrypted at rest with a per-document key reference. Content hashes prove a document has ' +
            ...
            },
            {
                ...
                layer: 'backend',
                plain_english: 'Turns the data into the reports operations, compliance, finance and a regulator each need. ' +
                    'The same report shows different detail depending on who asks.',
                talks_to: ['database'],
                security_note: 'Every export is recorded with a hash of what was produced and the masking profile that ' +
            ...
            },
            {
                ...
                layer: 'platform',
                plain_english: 'How we find out something is wrong. Structured logs, request tracing and security alerts.',
                talks_to: [],
                security_note: 'Every log line passes through a redaction layer. Passwords, tokens, full account numbers ' +
...
export async function decisionLog(db) {
    const decisions = await many(db, `SELECT decision_ref, title, decision_date, status, context, options_considered,
    ...
    return {
        ...
        note: 'A decision marked "awaiting_approval" is genuinely unresolved. The software runs with a ' +
            'placeholder in its place and says so wherever the placeholder is visible.',
    };
}
export async function approveDecision(db, decisionRef, approverName, reason, userId) {
    const existing = await maybeOne(db, 'SELECT status, title FROM decision_log WHERE decision_ref = $1', [decisionRef]);
    if (!existing)
    ...
    await recordAudit(db, {
        ...
        metadata: {
            ...
            note: 'Approving a decision here records the founder\'s choice. It does NOT change any configuration ' +
    ...
    return {
        ...
        next_step: 'Recording approval does not change the software. The placeholder this decision governs must ' +
...
export async function walkthrough(db, transactionId) {
    ...
    const steps = transitions.map((t) => {
        ...
        const definition = findTransition(t['from_state'], event) ?? null;
        const matchedEvent = Object.keys(ACTOR_OF).find((k) => {
            const d = findTransition(t['from_state'], k);
            return d?.to === t['to_state'];
        }) ?? '';
        const def = findTransition(t['from_state'], matchedEvent);
        return {
            from: t['from_state'],
            to: t['to_state'],
    ...
    const ledgerExplanation = journals.map((j) => ({
        ...
        entries: j['entries'].map((e) => ({
            ...
            amount: e['amount'],
            currency: e['currency'],
            what_this_means: e['direction'] === 'debit'
        ...
        how_a_reversal_would_work: `A reversal would post a new journal in which every debit above becomes a credit and every ` +
    ...
    return {
        ...
        partner_interactions: data['integration_events'].map((e) => ({
...
function explainDebit(category, accountType) {
    switch (category) {
...
function explainCredit(category, accountType) {
    switch (category) {
...
export async function assessment(db, moduleKey) {
    const module = await maybeOne(db, 'SELECT key, title FROM learning_module WHERE key = $1', [moduleKey]);
    if (!module)
        throw notFound('MODULE_NOT_FOUND', 'Module not found.');
    const questions = await many(db, 'SELECT id, ordinal, question, options FROM learning_assessment_question WHERE module_key = $1 ORDER BY ordinal', [moduleKey]);
    return {
        module: module.title,
...
export async function submitAssessment(db, moduleKey, userId, answers) {
    const questions = await many(db, `SELECT ordinal, question, options, correct_index, explanation
       FROM learning_assessment_question WHERE module_key = $1 ORDER BY ordinal`, [moduleKey]);
    if (questions.length === 0)
        throw notFound('MODULE_NOT_FOUND', 'No assessment for that module.');
    ...
    await db.query(`INSERT INTO learning_assessment_attempt (user_id, module_key, answers, score, total)
     VALUES ($1,$2,$3::jsonb,$4,$5)`, [userId, moduleKey, JSON.stringify(answers), score, questions.length]);
    return {
