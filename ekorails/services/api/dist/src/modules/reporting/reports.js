...
const dateWindow = (f) => [f.from ?? null, f.to ?? null];
export const REPORT_DEFINITIONS = [
    ...
    // -------------------------------------------------------------------------
    {
        ...
        family: 'operational',
        description: 'Transaction counts and values, success and failure rates, average processing and settlement ' +
        ...
        run: async (db, f) => {
            ...
            const rows = await many(db, `WITH scoped AS (
         ...
         SELECT send_currency AS currency,
           ...
           FROM scoped GROUP BY send_currency ORDER BY send_currency`, [from, to, f.organizationId ?? null, f.currency ?? null]);
            const queues = await one(db, `SELECT
           ...
           (SELECT count(*)::text FROM exception_case
            ...
            const settlementTime = await one(db, `SELECT round(avg(EXTRACT(EPOCH FROM (settled.occurred_at - submitted.occurred_at)) / 60.0), 2)::text AS avg_minutes
    ...
    },
    {
        ...
        run: async (db, f) => {
            const [from, to] = dateWindow(f);
            const rows = await many(db, `SELECT p.code AS partner_code, p.display_name AS partner_name, p.partner_role,
          ...
          GROUP BY p.id ORDER BY p.partner_role, p.code`, [from, to]);
            return {
                columns: [
                ...
                rows,
                summary: {
                    note: 'Every partner in this build is a simulator. Latency and failure figures reflect the ' +
    ...
    // -------------------------------------------------------------------------
    {
        ...
        family: 'compliance',
        description: 'Customers by risk rating, pending KYB cases, screening alerts, high-risk and rejected ' +
        ...
        run: async (db, f) => {
            const [from, to] = dateWindow(f);
            const rows = await many(db, `SELECT o.display_code AS organization_code, o.legal_name AS organization_name,
          ...
          ORDER BY
            ...
            o.display_code`, [from, to]);
            const decisions = await one(db, `SELECT
         ...
         FROM compliance_case c WHERE c.closed_at IS NOT NULL`);
            const ruleTriggers = await many(db, `SELECT rule_key,
    ...
    },
    {
        ...
        run: async (db, f) => {
            const [from, to] = dateWindow(f);
            const rows = await many(db, `SELECT c.reference AS case_reference, c.case_type, d.decision, d.reason,
          ...
          ORDER BY d.decided_at DESC LIMIT 5000`, [from, to, f.organizationId ?? null]);
            return {
                columns: [
                ...
                rows,
                summary: {
                    note: 'Compliance decisions are append-only. The application database role holds no UPDATE or ' +
    ...
    // -------------------------------------------------------------------------
    {
        ...
        filters: ['currency'],
        run: async (db, f) => {
            const rows = await many(db, `SELECT code AS account_code, name AS account_name, category, account_type, normal_side,
          ...
          ORDER BY currency, category, code`, [f.currency ?? null]);
            const totals = await many(db, 'SELECT currency, total_debits::text, total_credits::text, difference::text FROM trial_balance ORDER BY currency');
            return {
                columns: [
    ...
    },
    {
        ...
        family: 'financial',
        description: 'Simulated settlement values, fee revenue, partner costs, currency positions, suspense ' +
            'balances and reconciliation differences.',
        permission: 'report.financial.read',
        filters: ['from', 'to', 'currency'],
        run: async (db, f) => {
            const rows = await many(db, `SELECT currency,
           ...
           FROM ledger_account_balance
          WHERE ($1::text IS NULL OR currency = $1)
          GROUP BY currency ORDER BY currency`, [f.currency ?? null]);
            const settled = await many(db, `SELECT e.currency, sum(e.amount)::text AS settled_value, count(DISTINCT j.transaction_id)::text AS settled_count
          ...
          GROUP BY e.currency`, dateWindow(f));
            return {
                columns: [
                ...
                summary: {
                    ...
                    note: 'ALL FIGURES ARE SIMULATED. A non-zero fx_clearing_position is an open currency exposure; ' +
    ...
    // -------------------------------------------------------------------------
    {
        ...
        title: 'Pilot report',
        family: 'pilot',
        description: 'Participants, transactions initiated and completed, completion rate, average cost and ' +
            'processing time, exceptions, complaints, incidents, availability and limit breaches.',
        permission: 'report.pilot.read',
        filters: ['from', 'to'],
        run: async (db, f) => {
            const [from, to] = dateWindow(f);
            const participants = await many(db, `SELECT o.display_code AS organization_code, o.onboarding_status,
          ...
          GROUP BY o.id ORDER BY o.display_code`, [from, to]);
            const summary = await one(db, `SELECT
           ...
           (SELECT count(*)::text FROM rule_evaluation
            ...
            const cost = await many(db, `SELECT q.ekorails_fee_currency AS currency,
           ...
           FROM fx_quote q WHERE q.status = 'accepted' GROUP BY q.ekorails_fee_currency`);
            return {
                ...
                summary: {
                    ...
                    pilot_scope_note: 'Pilot duration, participant cap, transaction limits and success thresholds are UNCONFIRMED ' +
    ...
    // -------------------------------------------------------------------------
    {
        ...
        run: async (db, f) => {
            const [from, to] = dateWindow(f);
            const rows = await many(db, `SELECT t.reference, t.created_at, t.completed_at, t.state,
          ...
          ORDER BY t.created_at DESC LIMIT 10000`, [from, to, f.organizationId ?? null, f.corridor ?? null, f.currency ?? null]);
            return {
                columns: [
                ...
                rows,
                summary: {
                    note: 'Every transaction in this register was settled through a simulator. No real funds moved. ' +
    ...
    },
    {
        ...
        run: async (db, f) => {
            const [from, to] = dateWindow(f);
            const rows = await many(db, `SELECT r.reference, r.run_type, r.business_date, r.status,
          ...
          ORDER BY r.started_at DESC LIMIT 1000`, [from, to]);
            const breaks = await many(db, `SELECT exception_type, status, priority, count(*)::text AS n,
...
export async function run(db, key, filters, masking) {
 ...
 */
export async function regulatorOverview(db) {
    const scope = await many(db, `SELECT code, origin_country, destination_country, origin_currency, destination_currency,
    ...
    const participants = await many(db, `SELECT display_code, onboarding_status, COALESCE(risk_rating,'not_assessed') AS risk_rating,
            (suspended_at IS NOT NULL) AS suspended, created_at
       FROM organization WHERE kind = 'customer' ORDER BY display_code`);
    const activity = await one(db, `SELECT
       ...
       (SELECT count(*)::text FROM complaint) AS complaints,
       (SELECT count(*)::text FROM security_incident) AS security_incidents`);
    const controls = await many(db, `SELECT risk_ref, category, title, control_status, existing_controls, blocks_pilot
       FROM risk_register_entry ORDER BY blocks_pilot DESC, risk_ref`);
    const incidents = await many(db, `SELECT reference, title, severity, category, status, detected_at, resolved_at,
            personal_data_involved, notification_required, notified_at, is_simulated
       FROM security_incident ORDER BY detected_at DESC`);
    const chain = await one(db, 'SELECT count(*)::text AS n FROM verify_audit_chain()');
    const ledgerBalanced = await many(db, 'SELECT currency, difference::text FROM trial_balance');
    return {
        pilot_scope: {
            corridors: scope,
            status_note: 'Corridor, currencies and limits are UNCONFIRMED placeholders pending the CBN Regulatory ' +
        ...
        incidents,
        system_availability: 'NOT MEASURED. Uptime monitoring is not deployed in this build and no availability figure ' +
            'is claimed.',
        personal_data_note: 'This view deliberately shows organisation codes rather than names, and contains no ' +
