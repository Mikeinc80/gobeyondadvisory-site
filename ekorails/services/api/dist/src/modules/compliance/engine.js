...
import { RULES, SEVERITY_RANK, ACTION_RANK, } from './rules.js';
 ...
 */
async function loadActiveRules(db) {
    const rows = await many(db, `SELECT id, rule_key, version, parameters, severity, on_trigger_action, trigger_condition
       FROM risk_rule
      WHERE status = 'active'
      ORDER BY rule_key`);
    const pairs = [];
    ...
    for (const row of rows) {
        ...
        if (def)
            ...
            orphaned.push(`${row.rule_key}/v${row.version}`);
    }
    return { pairs, orphaned };
}
export async function evaluate(db, input, subject, actor = { userId: null, role: null }) {
    ...
    if (orphaned.length > 0) {
        ...
        throw new Error(`COMPLIANCE_ENGINE_RULE_GAP: ${orphaned.length} active rule(s) have no evaluator in this build: ` +
    ...
    for (const { row, def } of pairs) {
        ...
        if (!def.appliesTo.includes(subject.type)) {
            results.push({
                ...
                severity: null,
                action: null,
                message: `Not applicable to a ${subject.type} assessment. This rule applies to: ` +
                    `${def.appliesTo.join(', ')}.`,
    ...
    let outcomeSeverity = 'low';
    let recommendedAction = 'auto_continue';
    ...
    const score = triggered.reduce((sum, r) => sum + (r.severity ? (SEVERITY_RANK[r.severity] + 1) * 10 : 0), 0);
    ...
    const assessment = await one(db, `INSERT INTO risk_assessment (
    ...
    for (const { row } of pairs) {
        ...
        await db.query(`INSERT INTO rule_evaluation (
    ...
    let complianceCaseReference = null;
...
const SLA_HOURS = { critical: 4, high: 24, normal: 72, low: 120 };
async function openCaseForAssessment(db, input) {
    ...
    const caseType = specific
        ...
        : input.subjectType === 'transaction'
            ...
            : input.subjectType === 'beneficiary'
                ? 'beneficiary_review'
                : 'kyb_review';
    const priority = input.severity === 'prohibited' ? 'critical'
        : input.severity === 'high' ? 'high'
            : input.severity === 'medium' ? 'normal' : 'low';
    const requiresManager = input.action === 'enhanced_due_diligence' ||
        input.action === 'escalate' ||
        input.severity === 'prohibited' ||
        input.severity === 'high';
    const reference = await nextReference(db, 'compliance_case');
    const row = await one(db, `INSERT INTO compliance_case (
    ...
    await db.query(`INSERT INTO compliance_case_note (compliance_case_id, author_id, visibility, body)
 ...
 * for the decision.
 */
export async function buildTransactionInput(db, transactionId, deviceContext = { ipHash: null }) {
    const t = await one(db, `SELECT id, reference, organization_id, beneficiary_id, corridor_id, send_amount,
    ...
    const org = await one(db, `SELECT o.id, o.display_code, o.onboarding_status, o.risk_rating, o.suspended_at,
      ...
      WHERE o.id = $1`, [t.organization_id]);
    const ben = await one(db, `SELECT b.id, b.status, b.country, b.requires_rereview, b.created_at, b.first_used_at,
    ...
    const corridor = await one(db, `SELECT code, origin_country, destination_country, origin_currency, destination_currency,
       ...
       FROM corridor WHERE id = $1`, [t.corridor_id]);
    // Duplicate invoice candidates. Matched on fingerprint, excluding this transaction.
    const duplicates = t.invoice_fingerprint
        ? await many(db, `SELECT reference, state, created_at FROM transaction
          WHERE organization_id = $1 AND invoice_fingerprint = $2 AND id <> $3
          ORDER BY created_at DESC LIMIT 20`, [t.organization_id, t.invoice_fingerprint, t.id])
        : [];
    const docRoles = await many(db, 'SELECT DISTINCT role FROM transaction_document WHERE transaction_id = $1', [t.id]);
    ...
    const velocity = await one(db, `SELECT
     ...
     WHERE organization_id = $1 AND id <> $2 AND send_currency = $3
       AND state IN ${LIVE_STATES}`, [t.organization_id, t.id, t.send_currency]);
    const recent = await many(db, `SELECT send_amount::text FROM transaction
      WHERE organization_id = $1 AND id <> $2 AND send_currency = $3
        AND created_at >= now() - interval '24 hours'
        AND state IN ${LIVE_STATES}
      ORDER BY created_at DESC LIMIT 50`, [t.organization_id, t.id, t.send_currency]);
    ...
    const shared = await one(db, `SELECT count(DISTINCT b.organization_id)::text AS n
       FROM beneficiary b JOIN bank_account a ON a.id = b.bank_account_id
      WHERE a.identifier_fingerprint = $1 AND b.organization_id <> $2`, [ben.account_fingerprint, t.organization_id]);
    const recentBeneficiaries = await one(db, `SELECT count(*)::text AS n FROM beneficiary
      WHERE organization_id = $1 AND created_at >= now() - interval '7 days'`, [t.organization_id]);
    // Related party: does the beneficiary's legal name match a controller of the sender?
    const relatedParty = await one(db, `SELECT EXISTS (
       SELECT 1
     ...
     ) AS shares`, [t.organization_id, ben.id]);
    const initiator = await one(db, `SELECT (SELECT array_agg(role_code) FROM user_role WHERE user_id = $1
              AND (expires_at IS NULL OR expires_at > now())) AS roles`, [t.initiated_by]);
...
async function gatherScreening(db, subjects) {
    const rows = await many(db, `SELECT r.screening_type, c.status, r.match_score::text, r.matched_name, r.match_details
      ...
      ORDER BY c.requested_at DESC`, [[subjects.organizationId, subjects.beneficiaryId, subjects.transactionId]]);
    const pick = (type) => rows.filter((r) => r.screening_type === type);
    const sanctionsRows = pick('sanctions');
    const pepRows = pick('pep');
    const mediaRows = pick('adverse_media');
    const worstStatus = (candidates) => {
        const order = ['clear', 'false_positive', 'error', 'provider_unavailable', 'potential_match', 'confirmed_match'];
        let worst = 'clear';
        for (const c of candidates) {
            if (order.indexOf(c.status) > order.indexOf(worst))
                worst = c.status;
        }
    ...
    return {
        ...
        adverseMedia: {
            ...
            topics: mediaRows
                .flatMap((r) => (r.match_details?.['topics'] ?? [])),
        },
    };
}
async function gatherDeviceSignals(db, organizationId, ipHash) {
    const stats = await one(db, `SELECT
     ...
     WHERE u.organization_id = $1`, [organizationId, ipHash]);
    return {
        ipHash,
...
export async function recordDecision(db, input) {
    if (input.reason.trim().length < 20) {
        throw new Error('COMPLIANCE_REASON_TOO_SHORT: a compliance decision requires a written reason of at least ' +
            '20 characters. A decision nobody can explain later is not evidence.');
    }
    const row = await one(db, `INSERT INTO compliance_decision (
 ...
 */
export async function explainAssessment(db, riskAssessmentId) {
    const assessment = await maybeOne(db, `SELECT id, organization_id, subject_type, subject_id, outcome, recommended_action,
            score, evaluated_at, engine_version, ruleset_hash, input_hash, ruleset_snapshot
       FROM risk_assessment WHERE id = $1`, [riskAssessmentId]);
    if (!assessment)
        return null;
    const evaluations = await many(db, `SELECT rule_key, rule_version, triggered, evaluated_condition, parameters_used,
            data_used, result_severity, result_action, message, evaluated_at
       FROM rule_evaluation WHERE risk_assessment_id = $1 ORDER BY rule_key`, [riskAssessmentId]);
    const decisions = await many(db, `SELECT d.id, d.decision, d.reason, d.decided_by_role, d.decided_at, u.full_name AS decided_by_name
       FROM compliance_decision d
       LEFT JOIN app_user u ON u.id = d.decided_by
      WHERE d.risk_assessment_id = $1 ORDER BY d.decided_at`, [riskAssessmentId]);
    return {
        assessment,
        ...
        reproducibility_note: 'This record is self-contained. The rule text, parameter values and input data shown here ' +
