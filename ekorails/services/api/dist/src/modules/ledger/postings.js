 ...
 */
import { post, resolveAccount } from './ledger.js';
/**
 ...
 */
export async function postObligationRecognition(db, econ, postedBy) {
    const lines = [];
    ...
    return post(db, {
        ...
        plainEnglish: `The customer accepted the quote, so two things became true at once. They now owe us ` +
 ...
 */
export async function postFundingReceipt(db, econ, partners, received, postedBy) {
    ...
    return post(db, {
        ...
        plainEnglish: `The customer's funds arrived at the origin partner institution. The partner's account goes up ` +
 ...
 * USD leg:  Dr FX clearing (USD)                   Cr Customer settlement payable (USD)
 */
export async function postFxConversion(db, econ, postedBy) {
    ...
    return post(db, {
        ...
        plainEnglish: `The obligation changes currency. We stop owing ${econ.principalSend.toString()} ` +
 ...
 */
export async function postPartnerPositioning(db, econ, partners, postedBy) {
    const clearingSend = await resolveAccount(db, { category: 'fx_clearing', currency: econ.sendCurrency });
    const clearingReceive = await resolveAccount(db, { category: 'fx_clearing', currency: econ.receiveCurrency });
    const partnerFunding = await resolveAccount(db, {
    ...
    return post(db, {
        ...
        plainEnglish: `The funds physically move to match the converted obligation. ${econ.principalSend.toString()} ` +
 ...
 */
export async function postSettlementPayment(db, econ, partners, settled, postedBy) {
    const payableReceive = await resolveAccount(db, {
        category: 'customer_settlement_payable', currency: econ.receiveCurrency,
        organizationId: econ.organizationId,
    });
    ...
    return post(db, {
        ...
        plainEnglish: `The settlement partner paid ${settled.toString()} ${econ.receiveCurrency} to the beneficiary. ` +
 ...
 */
export async function postPartialSettlement(db, econ, partners, settled, postedBy) {
    ...
    return post(db, {
        ...
        plainEnglish: `The partner paid ${settled.toString()} ${econ.receiveCurrency} against an instruction for ` +
 ...
 */
export async function postReturnReceipt(db, econ, partners, returned, reason, postedBy) {
    const partnerSettlement = await resolveAccount(db, {
        category: 'partner_settlement_account', currency: econ.receiveCurrency,
        partnerId: partners.settlementPartnerId,
    });
    ...
    return post(db, {
        ...
        plainEnglish: `The beneficiary's bank returned ${returned.toString()} ${econ.receiveCurrency}. Reason given: ` +
...
export async function postTestLiquidity(db, input) {
    ...
    return post(db, {
        ...
        plainEnglish: `This gives a simulated partner an opening balance of ${input.amount.toString()} ` +
 ...
 */
export async function postReconciliationAdjustment(db, input) {
    ...
    return post(db, {
        ...
        plainEnglish: `Our records and the partner's records disagreed by ${input.amount.toString()} ` +
        ...
        lines: [
            ...
            {
                accountId: difference, direction: opposite, amount: input.amount,
...
export async function postPartnerFeePayment(db, econ, partners, postedBy) {
    if (!econ.partnerFee.isPositive())
        return null;
    const payable = await resolveAccount(db, {
    ...
    return post(db, {
        ...
        plainEnglish: `The fee we owed the partner, ${econ.partnerFee.toString()} ${econ.feeCurrency}, is paid out of ` +
