...
export function assertBalanced(lines) {
    ...
    for (const line of lines) {
        if (!line.amount.isPositive()) {
            throw new LedgerError('NON_POSITIVE_AMOUNT', `Line "${line.narrative}" has amount ${line.amount.toString()}. Entries carry a direction and a ` +
        ...
        if (line.direction === 'debit')
            bucket.debits = bucket.debits.add(line.amount);
        else
            bucket.credits = bucket.credits.add(line.amount);
        byCurrency.set(line.currency, bucket);
    }
    const totals = [];
    for (const [currency, bucket] of byCurrency) {
        if (!bucket.debits.equals(bucket.credits)) {
            throw new LedgerError('JOURNAL_IMBALANCE', `Journal does not balance in ${currency}: debits ${bucket.debits.toString()}, ` +
...
export async function post(db, request) {
    const totals = assertBalanced(request.lines);
    if (request.plainEnglish.trim().length < 20) {
        throw new LedgerError('MISSING_PLAIN_ENGLISH', 'Every journal must carry a plain-English explanation. A ledger nobody can read is a ledger ' +
            'nobody can audit.');
    }
    const reference = await nextReference(db, 'journal');
    const journal = await one(db, `INSERT INTO journal (
    ...
    for (const line of request.lines) {
        await db.query(`INSERT INTO journal_entry (
 ...
 */
export async function reverse(db, input) {
    if (input.reason.trim().length < 10) {
        throw new LedgerError('MISSING_REVERSAL_REASON', 'A reversal requires a written reason.');
    }
    const original = await one(db, `SELECT id, reference, journal_type, transaction_id, organization_id, description,
            posting_status, effective_date
       FROM journal WHERE id = $1`, [input.journalId]);
    ...
    const lines = await many(db, `SELECT ledger_account_id, direction, amount::text, currency, narrative,
            organization_id, transaction_id
       FROM journal_entry WHERE journal_id = $1 ORDER BY line_number`, [input.journalId]);
    const reversalReference = await nextReference(db, 'journal');
    const reversal = await one(db, `INSERT INTO journal (
    ...
    await db.query(`UPDATE journal SET posting_status = 'reversed', reversed_by_journal_id = $2 WHERE id = $1`, [original.id, reversal.id]);
    await recordAudit(db, {
        category: 'ledger_posting',
 ...
 */
export async function resolveAccount(db, key) {
    const code = accountCode(key);
    const existing = await maybeOne(db, 'SELECT id FROM ledger_account WHERE code = $1 AND currency = $2', [code, key.currency]);
    if (existing)
        return existing.id;
    const shape = ACCOUNT_SHAPE[key.category];
    const created = await one(db, `INSERT INTO ledger_account (code, name, category, normal_side, account_type, currency,
...
export async function accountBalances(db, filter = {}) {
    const rows = await many(db, `SELECT ledger_account_id, code, name, category, account_type, normal_side, currency,
      ...
      ORDER BY currency, category, code`, [filter.currency ?? null, filter.organizationId ?? null, filter.category ?? null]);
    return rows.map((r) => ({
...
export async function trialBalance(db) {
    const rows = await many(db, 'SELECT currency, total_debits::text, total_credits::text, difference::text FROM trial_balance ORDER BY currency');
    return rows.map((r) => ({
        currency: r.currency,
        totalDebits: r.total_debits,
        totalCredits: r.total_credits,
...
export async function journalsForTransaction(db, transactionId) {
    const journals = await many(db, `SELECT j.id, j.reference, j.journal_type, j.description, j.plain_english,
            j.effective_date, j.posted_at, j.posting_status, j.is_simulated,
            j.reverses_journal_id, j.reversed_by_journal_id
       FROM journal j WHERE j.transaction_id = $1 ORDER BY j.posted_at, j.reference`, [transactionId]);
    for (const journal of journals) {
        journal['entries'] = await many(db, `SELECT e.line_number, e.direction, e.amount::text AS amount, e.currency, e.narrative,
        ...
        WHERE e.journal_id = $1 ORDER BY e.line_number`, [journal['id']]);
    }
    return journals;
}
export async function verifyLedgerIntegrity(db) {
    const tb = await trialBalance(db);
    const unbalanced = await many(db, `SELECT j.reference, e.currency,
    ...
    const singleLine = await many(db, `SELECT j.reference FROM journal j
       JOIN journal_entry e ON e.journal_id = j.id
      GROUP BY j.reference HAVING count(*) < 2`);
    const counts = await one(db, `SELECT (SELECT count(*)::text FROM journal) AS journals,
