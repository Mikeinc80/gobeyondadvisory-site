...
import { settlementAdapter, partnerByRole, } from '../partners/adapters.js';
import { queueNotification } from '../notification/notify.js';
import { openExceptionCase } from '../recon/exceptions.js';
async function loadTransaction(db, transactionId) {
    const row = await maybeOne(db, `SELECT id, reference, organization_id, beneficiary_id, state, send_amount::text,
            send_currency, receive_currency, expected_receive_amount::text, fx_quote_id, initiated_by
       FROM transaction WHERE id = $1`, [transactionId]);
    if (!row)
 ...
 */
export async function economicsFor(db, transactionId) {
    const txn = await loadTransaction(db, transactionId);
    if (!txn.fx_quote_id) {
        throw precondition('NO_ACCEPTED_QUOTE', 'This transaction has no quote. Its economics are undefined until a quote is accepted.');
    }
    const quote = await one(db, `SELECT send_amount::text, total_payable::text, expected_receivable::text,
            ekorails_fee::text, partner_fee::text, tax_or_levy::text,
            ekorails_fee_currency, provider_rate::text, status
       FROM fx_quote WHERE id = $1`, [txn.fx_quote_id]);
    return {
...
async function resolvePartners(db) {
    ...
    if (!origin || !settlement) {
        throw precondition('PARTNER_NOT_CONFIGURED', 'No origin or settlement partner is configured. Settlement cannot be routed without both.');
    }
    return { originPartnerId: origin.id, settlementPartnerId: settlement.id };
}
async function notifyEdge(db, result, transactionId, organizationId) {
    const EVENT_BY_EDGE = {
    ...
    for (const roleCode of result.definition.notifies) {
        await queueNotification(db, {
            ...
            recipientUserId: roleCode === 'initiator'
                ? (await one(db, 'SELECT initiated_by FROM transaction WHERE id = $1', [transactionId])).initiated_by
                : null,
            channel: 'in_app',
            eventType: eventType,
            transactionId,
            ...
            body: `Transaction ${result.reference} moved to "${result.to.replace(/_/g, ' ')}". ` +
...
export async function acceptQuoteAndRecogniseObligation(db, transactionId, actor) {
...
export async function requestFunding(db, transactionId, actor) {
    const econ = await economicsFor(db, transactionId);
    const partners = await resolvePartners(db);
    await db.query(`INSERT INTO funding_instruction (
    ...
    const result = await transition(db, {
        ...
        actorPermissions: actor.permissions,
        stepUpValid: actor.stepUpValid ?? false,
        reason: `Funding instruction issued for ${econ.totalPayable.toString()} ${econ.sendCurrency}. ` +
...
export async function confirmFunding(db, transactionId, actor) {
    const econ = await economicsFor(db, transactionId);
    const partners = await resolvePartners(db);
    const partner = await one(db, 'SELECT adapter_key FROM partner WHERE id = $1', [partners.originPartnerId]);
    const ctx = {
    ...
    const confirmation = await adapter.confirmFunding(ctx, econ.totalPayable, econ.sendCurrency, `FUND-${econ.transactionReference}`);
    if (confirmation.status === 'awaiting') {
        // Delayed funding is not a failure; it is a wait. The transaction stays where it is.
        await db.query("UPDATE funding_instruction SET status = 'awaiting' WHERE transaction_id = $1", [transactionId]);
    ...
    await db.query(`UPDATE funding_instruction
    ...
    if (isShort) {
        ...
        await openExceptionCase(db, {
            ...
            priority: 'high',
            detail: `Expected ${econ.totalPayable.toString()} ${econ.sendCurrency}, received ${received.toString()}. ` +
 ...
 */
export async function prepareSettlement(db, transactionId, actor) {
    const econ = await economicsFor(db, transactionId);
    const partners = await resolvePartners(db);
    ...
    const guard = await one(db, `SELECT
       ...
       (SELECT o.suspended_at IS NULL AND o.onboarding_status = 'approved'
         ...
         WHERE t.id = $1) AS org_ok`, [transactionId]);
    ...
    if (!guard.beneficiary_ok) {
        throw precondition('BENEFICIARY_NO_LONGER_APPROVED', 'The beneficiary is no longer approved or has been changed since approval. Settlement is blocked.');
    }
    if (!guard.org_ok) {
        throw precondition('ORGANISATION_NOT_ELIGIBLE', 'The organisation is suspended or is no longer approved. Settlement is blocked.');
    }
    ...
    await db.query(`INSERT INTO settlement_instruction (
    ...
    const result = await transition(db, {
        ...
        actorPermissions: actor.permissions,
        stepUpValid: actor.stepUpValid ?? false,
        reason: `Obligation converted at ${econ.rate.toString()} and liquidity positioned with the settlement ` +
            `partner (simulated).`,
        evidence: {
 ...
 */
export async function submitSettlement(db, transactionId, actor) {
    const econ = await economicsFor(db, transactionId);
    const partners = await resolvePartners(db);
    const journalReferences = [];
    let exceptionReference = null;
    const beneficiary = await one(db, `SELECT b.legal_name, b.country, a.identifier_last4 AS last4, b.payment_purpose AS purpose
    ...
    const partner = await one(db, 'SELECT adapter_key, display_name FROM partner WHERE id = $1', [partners.settlementPartnerId]);
    ...
    await db.query(`UPDATE settlement_instruction SET status = 'submitted', submitted_at = now(), released_by = $2
      WHERE transaction_id = $1 AND idempotency_key = $3`, [transactionId, actor.userId, idempotencyKey]);
    const ctx = {
    ...
    await db.query(`UPDATE settlement_instruction
    ...
    switch (outcome.status) {
        case 'settled': {
            const journal = await postings.postSettlementPayment(db, econ, partners, outcome.settledAmount ?? econ.principalReceive, actor.userId);
            journalReferences.push(journal.reference);
            await db.query('UPDATE transaction SET actual_receive_amount = $2 WHERE id = $1', [transactionId, (outcome.settledAmount ?? econ.principalReceive).toString()]);
            const result = await transition(db, {
                transactionId, event: 'partner_settled_immediate', actorType: 'partner',
                actorPartnerId: partners.settlementPartnerId,
                reason: `Partner reported settlement of ${(outcome.settledAmount ?? econ.principalReceive).toString()} ` +
        ...
        case 'partially_settled': {
            ...
            journalReferences.push(journal.reference);
            await db.query('UPDATE transaction SET actual_receive_amount = $2 WHERE id = $1', [transactionId, settled.toString()]);
            ...
            const result = await transition(db, {
                ...
                actorPartnerId: partners.settlementPartnerId,
                reason: `Partner settled ${settled.toString()} of ${econ.principalReceive.toString()} ` +
            ...
            const exc = await openExceptionCase(db, {
                ...
                priority: 'high',
                detail: `Partial settlement on ${econ.transactionReference}. Instructed ` +
        ...
        case 'returned': {
            const settlementJournal = await postings.postSettlementPayment(db, econ, partners, econ.principalReceive, actor.userId);
            ...
            const returnJournal = await postings.postReturnReceipt(db, econ, partners, econ.principalReceive, outcome.failureDetail ?? 'Returned by destination institution', actor.userId);
            ...
            const exc = await openExceptionCase(db, {
                ...
                priority: 'high',
                detail: `Payment returned on ${econ.transactionReference}. Funds are back with the settlement partner ` +
        ...
        case 'timeout': {
            ...
            const journal = await ledger.post(db, {
                ...
                plainEnglish: `We sent a settlement instruction and the partner did not respond. We do not know whether ` +
            ...
            const result = await transition(db, {
                ...
                reason: `Partner did not respond to instruction ${idempotencyKey}. Outcome UNKNOWN. Automatic retry ` +
            ...
            const exc = await openExceptionCase(db, {
                ...
                currency: econ.receiveCurrency, amount: econ.principalReceive,
                priority: 'critical',
                detail: `Instruction ${idempotencyKey} timed out with no response. Establish with the partner whether ` +
        ...
        case 'failed': {
            ...
            const positioningJournal = await maybeOne(db, `SELECT id, reference FROM journal
          WHERE transaction_id = $1 AND journal_type = 'partner_positioning' AND posting_status = 'posted'
          ORDER BY posted_at DESC LIMIT 1`, [transactionId]);
            const conversionJournal = await maybeOne(db, `SELECT id, reference FROM journal
        ...
        case 'duplicate_ignored': {
            ...
            throw conflict('DUPLICATE_SETTLEMENT_INSTRUCTION', 'This settlement has already been instructed. A second instruction was not sent.', { idempotency_key: idempotencyKey, detail: outcome.failureDetail });
        }
        case 'accepted':
        case 'processing':
        default: {
...
export async function confirmBeneficiaryCredit(db, transactionId, partnerId) {
    const txn = await loadTransaction(db, transactionId);
    const result = await transition(db, {
...
export async function complete(db, transactionId, actor) {
    const econ = await economicsFor(db, transactionId);
    const partners = await resolvePartners(db);
    const openBreaks = await one(db, `SELECT count(*)::text AS n FROM exception_case
      WHERE transaction_id = $1 AND status NOT IN ('resolved','written_off','closed_no_action')`, [transactionId]);
    if (Number(openBreaks.n) > 0) {
        throw precondition('OPEN_EXCEPTION', `This transaction has ${openBreaks.n} open exception(s). It cannot be marked complete until they ` +
            `are resolved.`);
    }
...
export async function timeline(db, transactionId) {
    ...
    const transitions = await many(db, `SELECT tt.from_state, tt.to_state, tt.actor_type, tt.actor_role, tt.reason,
      ...
      WHERE tt.transaction_id = $1 ORDER BY tt.occurred_at, tt.id`, [transactionId]);
    const journals = await ledger.journalsForTransaction(db, transactionId);
    const integrationEvents = await many(db, `SELECT operation, direction, outcome, http_status, latency_ms, simulation_scenario,
            occurred_at, request_payload, response_payload
       FROM integration_event WHERE transaction_id = $1 ORDER BY occurred_at`, [transactionId]);
    const auditEvents = await many(db, `SELECT seq::text AS seq, occurred_at, category, action, outcome, actor_role, reason
       FROM audit_event WHERE transaction_id = $1 ORDER BY seq`, [transactionId]);
    const notifications = await many(db, `SELECT channel, event_type, subject, status, created_at
       FROM notification WHERE transaction_id = $1 ORDER BY created_at`, [transactionId]);
    const exceptions = await many(db, `SELECT reference, exception_type, status, priority, amount::text AS amount, currency, opened_at
       FROM exception_case WHERE transaction_id = $1 ORDER BY opened_at`, [transactionId]);
    return {
        transaction: txn,
        transitions,
        journals,
