...
export function realmsForRoles(roleCodes) {
    const out = new Set();
    for (const code of roleCodes) {
        const role = ROLES[code];
        if (role)
 ...
 */
export function databaseScopeForRoles(roleCodes) {
    const realms = realmsForRoles(roleCodes);
    const crossOrg = ['backoffice', 'external', 'platform'];
...
export function requiresStepUp(roleCodes) {
    return roleCodes.some((c) => ROLES[c]?.requiresStepUp === true);
}
export function isReadOnlyRole(roleCodes) {
    return roleCodes.length > 0 && roleCodes.every((c) => ROLES[c]?.realm === 'external');
}
export function maskingProfileForRoles(roleCodes) {
    const perms = permissionsForRoles(roleCodes);
    if (perms.has('pii.unmask'))
        return 'full';
    if (roleCodes.some((c) => ROLES[c]?.realm === 'external'))
        return 'masked';
    if (roleCodes.some((c) => ROLES[c]?.realm === 'platform'))
        return 'masked';
    return 'operational';
}
