...
import { hashPassword, verifyPassword, randomToken, sha256Hex, safeEqual, encryptField, decryptField, verifyTotp, generateTotpSecret, totpProvisioningUri, checkPasswordPolicy, } from '../core/crypto.js';
import { recordAudit } from '../audit/audit.js';
import { unauthenticated, forbidden, invalid, precondition } from '../core/errors.js';
import { databaseScopeForRoles, permissionsForRoles } from './rbac.js';
...
export async function login(db, req) {
    ...
    const user = await maybeOne(db, `SELECT u.id, u.organization_id, u.email, u.full_name, u.display_name,
      ...
      WHERE u.email_normalised = $1`, [emailNorm]);
    const recordFailure = async (reason) => {
        await db.query(`INSERT INTO login_attempt (email_hash, user_id, succeeded, failure_reason, ip_hash, user_agent_hash)
       VALUES ($1, $2, false, $3, $4, $5)`, [emailHash, user?.id ?? null, reason, req.ipHash, req.userAgentHash]);
    ...
    if (!user.password_hash || !verifyPassword(req.password, user.password_hash)) {
        ...
        await db.query(`UPDATE app_user
        ...
        WHERE id = $1`, [user.id, attempts, lock, String(LOCKOUT_MS)]);
    ...
    await db.query('UPDATE app_user SET failed_login_count = 0, locked_until = NULL, last_login_at = now(), last_login_ip_hash = $2 WHERE id = $1', [user.id, req.ipHash]);
    const sessionToken = randomToken(32);
    const csrfToken = randomToken(32);
    const now = Date.now();
    const session = await one(db, `INSERT INTO user_session (
    ...
    await db.query(`INSERT INTO login_attempt (email_hash, user_id, succeeded, ip_hash, user_agent_hash)
     VALUES ($1, $2, true, $3, $4)`, [emailHash, user.id, req.ipHash, req.userAgentHash]);
    await recordAudit(db, {
        category: 'authentication',
        action: 'login',
        outcome: 'success',
...
export async function verifyMfa(db, sessionToken, code, ipHash) {
    const row = await maybeOne(db, `SELECT s.id AS session_id, u.id AS user_id, u.organization_id,
      ...
      WHERE s.token_hash = $1 AND s.revoked_at IS NULL AND s.absolute_expiry > now()`, [sha256Hex(sessionToken)]);
    ...
    if (!result.valid) {
        ...
        throw unauthenticated(result.reason === 'replayed' ? 'MFA_CODE_REPLAYED' : 'MFA_CODE_INVALID', result.reason === 'replayed'
    ...
    await db.query(`UPDATE user_session
        SET mfa_satisfied = true, step_up_until = now() + ($2 || ' milliseconds')::interval, last_seen_at = now()
      WHERE id = $1`, [row.session_id, String(STEP_UP_LIFETIME_MS)]);
    await recordAudit(db, {
        category: 'authentication',
        action: 'mfa.verify',
        outcome: 'success',
...
export async function beginMfaEnrolment(db, userId, email) {
...
export async function completeMfaEnrolment(db, userId, organizationId, code) {
    const row = await one(db, 'SELECT mfa_secret_encrypted FROM app_user WHERE id = $1', [userId]);
    ...
    await db.query('UPDATE app_user SET mfa_enrolled = true, mfa_last_used_step = $2 WHERE id = $1', [userId, result.step]);
    await recordAudit(db, {
        category: 'authentication',
        action: 'mfa.enrol',
        outcome: 'success',
 ...
 */
export async function resolveSession(db, sessionToken) {
    const row = await maybeOne(db, `SELECT s.id AS session_id, s.mfa_satisfied, s.step_up_until, s.idle_expiry,
       ...
       JOIN organization o ON o.id = u.organization_id
      WHERE s.token_hash = $1`, [sha256Hex(sessionToken)]);
    if (!row)
        throw unauthenticated('SESSION_INVALID', 'Sign in to continue.');
    if (row.revoked_at)
        throw unauthenticated('SESSION_REVOKED', 'This session has ended.');
    ...
    if (row.idle_expiry.getTime() <= Date.now()) {
        await db.query("UPDATE user_session SET revoked_at = now(), revoked_reason = 'idle_timeout' WHERE id = $1", [row.session_id]);
    ...
    await db.query(`UPDATE user_session
      ...
      WHERE id = $1`, [row.session_id, String(SESSION_IDLE_LIFETIME_MS)]);
    return {
        userId: row.user_id,
        organizationId: row.organization_id,
        email: row.email,
...
export async function verifyCsrf(db, sessionToken, presentedCsrfToken) {
    const row = await maybeOne(db, 'SELECT csrf_token_hash FROM user_session WHERE token_hash = $1 AND revoked_at IS NULL', [sha256Hex(sessionToken)]);
    if (!row)
        return false;
    return safeEqual(row.csrf_token_hash, sha256Hex(presentedCsrfToken));
}
export async function logout(db, sessionId, userId, organizationId) {
    await db.query("UPDATE user_session SET revoked_at = now(), revoked_reason = 'user_logout' WHERE id = $1 AND revoked_at IS NULL", [sessionId]);
    await recordAudit(db, {
...
export async function stepUp(db, user, code) {
    const row = await one(db, 'SELECT mfa_secret_encrypted, mfa_last_used_step::text AS mfa_last_used_step FROM app_user WHERE id = $1', [user.userId]);
    if (!row.mfa_secret_encrypted) {
        throw precondition('MFA_NOT_ENROLLED', 'Enrol in multi-factor authentication before performing this action.');
    }
    const result = verifyTotp(decryptField(row.mfa_secret_encrypted), code, row.mfa_last_used_step === null ? null : Number(row.mfa_last_used_step));
    if (!result.valid) {
        await recordAudit(db, {
    ...
    await db.query(`UPDATE user_session SET step_up_until = now() + ($2 || ' milliseconds')::interval WHERE id = $1`, [user.sessionId, String(STEP_UP_LIFETIME_MS)]);
    await recordAudit(db, {
...
export async function changePassword(db, user, currentPassword, newPassword) {
    const row = await one(db, 'SELECT password_hash, password_history FROM app_user WHERE id = $1', [user.userId]);
    ...
    await db.query(`UPDATE app_user
      ...
      WHERE id = $1`, [user.userId, hashPassword(newPassword), JSON.stringify(history)]);
    ...
    await db.query(`UPDATE user_session SET revoked_at = now(), revoked_reason = 'password_changed'
      WHERE user_id = $1 AND id <> $2 AND revoked_at IS NULL`, [user.userId, user.sessionId]);
    await recordAudit(db, {
...
export async function createUser(db, input) {
    ...
    const user = await one(db, `INSERT INTO app_user (
    ...
    for (const role of input.roles) {
        await db.query('INSERT INTO user_role (user_id, role_code) VALUES ($1, $2) ON CONFLICT DO NOTHING', [user.id, role]);
    }
    await recordAudit(db, {
