 ...
 */
import { Router, field, isNonEmptyString, isUuid, isDecimalString, isCurrency, isObject, isArray, oneOf, serialiseCookie } from './router.js';
import { withContext, withReadOnlyContext } from '../db/pool.js';
...
function contextFor(ctx) {
    const user = ctx.user;
    if (!user)
        return { scope: 'none', requestId: ctx.requestId, correlationId: ctx.correlationId };
    return {
        scope: user.scope,
...
function requireUser(ctx) {
    if (!ctx.user)
        throw forbidden('SESSION_REQUIRED', 'Sign in to continue.');
    return ctx.user;
}
function approvalActor(ctx) {
...
function targetOrganization(ctx, explicit) {
    const user = requireUser(ctx);
    if (user.scope === 'org')
        return user.organizationId;
    if (!explicit)
        throw invalid('ORGANIZATION_REQUIRED', 'Specify the organisation.');
    return explicit;
}
export function buildRouter() {
    ...
    router.register({
        ...
        handler: async () => ({
            entity: 'EKORails LTD',
            what_this_software_is: 'A compliance-first orchestration layer for business-to-business cross-border trade ' +
            ...
            unresolved_facts: {
                note: 'The CBN Regulatory Sandbox application was not available to this build. No regulatory or ' +
    ...
    router.register({
        ...
        handler: async (ctx) => {
            ...
            await withContext({ scope: 'system' }, (db) => auth.verifyMfa(db, ctx.sessionToken, code, ctx.ipHash));
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            return withContext(contextFor(ctx), (db) => auth.beginMfaEnrolment(db, user.userId, user.email));
        },
    });
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            const code = field(ctx.body, 'code', isNonEmptyString, 'a 6-digit code');
            await withContext(contextFor(ctx), (db) => auth.completeMfaEnrolment(db, user.userId, user.organizationId, code));
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            await withContext(contextFor(ctx), (db) => auth.logout(db, user.sessionId, user.userId, user.organizationId));
            ctx.setCookies.push(serialiseCookie('ekorails_session', '', { maxAgeSeconds: 0 }), serialiseCookie('ekorails_csrf', '', { maxAgeSeconds: 0, httpOnly: false }));
    ...
    router.register({
        ...
        handler: async (ctx) => {
            ...
            return withReadOnlyContext(contextFor(ctx), async (db) => {
                const org = await one(db, `SELECT id, display_code, legal_name, trading_name, onboarding_status, risk_rating,
                  suspended_at, suspension_reason, created_at
             FROM organization WHERE id = $1`, [orgId]);
                const profile = await maybeOne(db, `SELECT id, version, legal_business_name, trading_name, registration_number, jurisdiction,
             ...
             FROM organization_profile WHERE organization_id = $1 AND is_current`, [orgId]);
                const people = await many(db, `SELECT p.id, p.full_name, p.nationality, p.country_of_residence, p.is_pep,
             ...
             FROM natural_person p WHERE p.organization_id = $1 ORDER BY p.full_name`, [orgId]);
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            const body = (ctx.body ?? {});
            return withContext(contextFor(ctx), (db) => orgService.upsertKybProfile(db, {
                organizationId: user.organizationId,
                userId: user.userId,
                profile: {
                    legalBusinessName: field(body, 'legal_business_name', isNonEmptyString, 'the legal business name'),
                    tradingName: body['trading_name'] ?? null,
                    ...
                    taxIdentificationNumber: body['tax_identification_number'] ?? null,
                    regulatoryLicence: body['regulatory_licence'] ?? null,
                    expectedCorridors: body['expected_corridors'] ?? [],
                    expectedMonthlyVolume: body['expected_monthly_volume'] ?? null,
                    expectedMonthlyCurrency: body['expected_monthly_currency'] ?? null,
                    expectedTransactionSize: body['expected_transaction_size'] ?? null,
                    expectedTxnCurrency: body['expected_txn_currency'] ?? null,
    ...
    router.register({
        ...
        handler: async (ctx) => {
            ...
            return withContext(contextFor(ctx), async (db) => {
                const profile = await one(db, 'SELECT id FROM organization_profile WHERE organization_id = $1 AND is_current', [user.organizationId]);
                return orgService.addPerson(db, {
                    ...
                    person: {
                        ...
                        countryOfResidence: body['country_of_residence'] ?? null,
                        residentialAddress: body['residential_address'] ?? null,
                        idDocumentType: body['id_document_type'] ?? null,
                        idNumber: body['id_number'] ?? null,
                        idExpiresOn: body['id_expires_on'] ?? null,
                        isPep: body['is_pep'] === true,
                        pepDeclaration: body['pep_declaration'] ?? null,
                        pepCategory: body['pep_category'] ?? null,
                        capacities: field(body, 'capacities', isArray, 'a list of capacities'),
                    },
                });
            });
        },
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            return withContext(contextFor(ctx), (db) => orgService.submitForKybReview(db, user.organizationId, user.userId));
        },
    });
    // -------------------------------------------------------------------------
    // Documents
    ...
    router.register({
        ...
        handler: async (ctx) => {
            ...
            const bytes = Buffer.isBuffer(ctx.body)
                ? ctx.body
                : Buffer.from(String(ctx.body?.['content_base64'] ?? ''), 'base64');
            const documentType = ctx.query.get('document_type')
                ?? ctx.body?.['document_type'];
            ...
            return withContext(contextFor(ctx), (db) => orgService.uploadDocument(db, {
    ...
    router.register({
        ...
        handler: async (ctx) => {
            ...
            if (user.scope === 'global' && maskingProfileForRoles(user.roles) === 'masked' && !reason) {
                throw precondition('ACCESS_REASON_REQUIRED', 'Accessing a customer document in this role requires a stated reason, which is recorded.');
            }
            return withContext(contextFor(ctx), (db) => orgService.mintDocumentUrl(db, {
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            const body = (ctx.body ?? {});
            const result = await withContext(contextFor(ctx), (db) => orgService.proposeExtraction(db, {
                documentId: ctx.params['id'], organizationId: user.organizationId,
                extractor: body['extractor'] ?? 'ekorails-extract-stub',
                extractorVersion: body['extractor_version'] ?? '0.1.0',
                proposedFields: field(body, 'proposed_fields', isObject, 'an object'),
                fieldConfidence: body['field_confidence'] ?? {},
            }));
            return {
                ...result,
                status: 'proposed',
                notice: 'These fields are a proposal. They are not used by the compliance engine and have no effect ' +
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            const body = (ctx.body ?? {});
            await withContext(contextFor(ctx), (db) => orgService.confirmExtraction(db, {
    ...
    router.register({
        ...
        handler: async (ctx) => {
            ...
            return withContext(contextFor(ctx), (db) => orgService.createBeneficiary(db, {
                ...
                beneficiary: {
                    ...
                    supportingContractId: body['supporting_contract_id'] ?? null,
                    bank: {
                        ...
                        swiftBic: bank['swift_bic'] ?? null,
                        identifierScheme: field(bank, 'identifier_scheme', oneOf('iban', 'nuban', 'account_number', 'sort_code_account', 'other'), 'a supported identifier scheme'),
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            return withContext(contextFor(ctx), (db) => orgService.updateBeneficiary(db, {
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            const body = (ctx.body ?? {});
            return withContext(contextFor(ctx), (db) => orgService.reviewBeneficiary(db, {
                beneficiaryId: ctx.params['id'],
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            return withReadOnlyContext(contextFor(ctx), (db) => txnService.listTransactions(db, {
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            return withReadOnlyContext(contextFor(ctx), (db) => txnService.requiringAction(db, user.organizationId, user.userId, user.permissions));
        },
    });
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            const body = (ctx.body ?? {});
            return withContext(contextFor(ctx), (db) => txnService.createTransaction(db, {
                organizationId: user.organizationId,
                ...
                requestedSettlementDate: body['requested_settlement_date'] ?? null,
                invoiceNumber: body['invoice_number'] ?? null,
                documentLinks: body['documents'] ?? [],
                initiatedBy: user.userId,
                idempotencyKey: ctx.headers['idempotency-key'] ?? body['idempotency_key'] ?? null,
            }));
        },
    });
    router.register({
    ...
    router.register({
        ...
        tags: ['transactions'],
        handler: async (ctx) => withContext(contextFor(ctx), (db) => txnService.submitForApproval(db, ctx.params['id'], approvalActor(ctx))),
    });
    router.register({
        ...
        handler: async (ctx) => {
            const body = (ctx.body ?? {});
            return withContext(contextFor(ctx), (db) => txnService.businessApprove(db, {
                transactionId: ctx.params['id'],
                actor: approvalActor(ctx),
                approve: body['approve'] !== false,
                reason: body['reason'] ?? 'Authorised.',
            }));
        },
    });
    router.register({
        ...
        handler: async (ctx) => {
            const body = (ctx.body ?? {});
            return withContext(contextFor(ctx), (db) => txnService.complianceDecide(db, {
                transactionId: ctx.params['id'],
                actor: approvalActor(ctx),
                decision: field(body, 'decision', oneOf('approve', 'reject', 'request_information', 'suspend'), 'a decision'),
    ...
    router.register({
        ...
        handler: async (ctx) => {
            ...
            const body = (ctx.body ?? {});
            return withContext(contextFor(ctx), async (db) => {
                const txn = await one(db, `SELECT id, organization_id, corridor_id, state, send_amount::text, send_currency, receive_currency
             FROM transaction WHERE id = $1`, [ctx.params['id']]);
                if (txn.state !== 'compliance_approved') {
                    throw precondition('NOT_COMPLIANCE_APPROVED', `A quote can only be issued once compliance has approved. This transaction is "${txn.state}".`);
                }
                const providerRate = Decimal.fromString(field(body, 'provider_rate', isNonEmptyString, 'a rate as a decimal string'), RATE_SCALE);
                const referenceRate = Decimal.fromString(body['reference_rate'] ?? providerRate.toString(), RATE_SCALE);
                const quote = await fx.issueQuote(db, {
                    ...
                    referenceRate,
                    referenceRateSource: body['reference_rate_source']
                        ?? 'Manually entered by an authorised treasury operator',
                    referenceRateAt: new Date(),
                    providerRate,
                    quoteSource: body['quote_source'] ?? 'manual_treasury_entry',
                    quoteSourceDetail: body['quote_source_detail'] ?? null,
                ...
                await db.query('UPDATE transaction SET expected_receive_amount = $2 WHERE id = $1', [txn.id, quote.expectedReceivable]);
    ...
    router.register({
        ...
        handler: async (ctx) => {
            ...
            if (!actor.stepUpValid) {
                throw forbidden('STEP_UP_REQUIRED', 'Confirm your identity with your authenticator before accepting a quote.', 'quote_accept requires step-up');
            }
            return withContext(contextFor(ctx), async (db) => {
                const accepted = await fx.acceptQuote(db, {
                    quoteId: ctx.params['id'], acceptedBy: user.userId, organizationId: user.organizationId,
                });
                const txn = await one(db, 'SELECT id FROM transaction WHERE fx_quote_id = $1', [ctx.params['id']]);
                const result = await settlement.acceptQuoteAndRecogniseObligation(db, txn.id, {
                    ...
                    permissions: user.permissions, stepUpValid: true,
                });
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            return withContext(contextFor(ctx), (db) => settlement.confirmFunding(db, ctx.params['id'], {
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            return withContext(contextFor(ctx), (db) => settlement.prepareSettlement(db, ctx.params['id'], {
                type: 'user', userId: user.userId, role: user.roles[0] ?? 'treasury_operator',
                permissions: user.permissions, stepUpValid: true,
            }));
        },
    ...
    router.register({
        ...
        handler: async (ctx) => {
            ...
            const actor = approvalActor(ctx);
            if (!actor.stepUpValid) {
                throw forbidden('STEP_UP_REQUIRED', 'Confirm your identity with your authenticator before releasing a settlement.', 'submit_to_partner requires step-up');
            }
            return withContext(contextFor(ctx), (db) => settlement.submitSettlement(db, ctx.params['id'], {
                type: 'user', userId: user.userId, role: user.roles[0] ?? 'treasury_operator',
                permissions: user.permissions, stepUpValid: true,
            }));
        },
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            return withContext(contextFor(ctx), (db) => settlement.complete(db, ctx.params['id'], {
    ...
    router.register({
        ...
        tags: ['compliance'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => many(db, `SELECT c.reference, c.case_type, c.status, c.priority, c.opened_at, c.sla_due_at,
            ...
            LIMIT 200`, [ctx.query.get('status')])),
    });
    router.register({
        ...
        tags: ['compliance'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), async (db) => {
            const c = await maybeOne(db, `SELECT c.*, o.legal_name AS organization_name, o.display_code AS organization_code
             FROM compliance_case c JOIN organization o ON o.id = c.organization_id
            WHERE c.reference = $1`, [ctx.params['reference']]);
            if (!c)
                throw notFound('CASE_NOT_FOUND', 'Case not found.');
            const assessment = c['risk_assessment_id']
                ? await explainAssessment(db, c['risk_assessment_id']) : null;
            const notes = await many(db, `SELECT n.body, n.visibility, n.created_at, u.display_name AS author_name
             FROM compliance_case_note n LEFT JOIN app_user u ON u.id = n.author_id
            WHERE n.compliance_case_id = $1 ORDER BY n.created_at`, [c['id']]);
            ...
            ORDER BY sc.requested_at DESC LIMIT 20`, [c['subject_id'], c['organization_id']]);
    ...
    router.register({
        ...
        handler: async (ctx) => {
            ...
            return withContext(contextFor(ctx), async (db) => {
                const c = await one(db, `SELECT id, organization_id, risk_assessment_id, requires_manager, status
             FROM compliance_case WHERE reference = $1`, [ctx.params['reference']]);
                ...
                if (c.requires_manager && !isManager && ['cleared', 'approved'].includes(decision)) {
                    ...
                    throw forbidden('MANAGER_APPROVAL_REQUIRED', 'This case is flagged for manager approval and cannot be cleared by an analyst.');
                }
                // A manager cannot review their own earlier decision on the same case.
                const priorByMe = await maybeOne(db, `SELECT id FROM compliance_decision
            WHERE compliance_case_id = $1 AND decided_by = $2 ORDER BY decided_at LIMIT 1`, [c.id, user.userId]);
                ...
                const nextStatus = decision === 'cleared' || decision === 'cleared_false_positive' || decision === 'approved'
                    ? 'closed_cleared'
            ...
            WHERE id = $1`, [c.id, nextStatus, user.userId]);
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            const body = (ctx.body ?? {});
            await withContext(contextFor(ctx), (db) => orgService.disposeScreening(db, {
                screeningCaseId: ctx.params['id'],
                disposition: field(body, 'disposition', oneOf('cleared', 'escalated', 'blocked', 'pending_review'), 'a disposition'),
                reason: field(body, 'reason', isNonEmptyString, 'a written reason'),
                userId: user.userId, role: user.roles[0] ?? 'compliance_analyst',
            }));
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            const body = (ctx.body ?? {});
            return withContext(contextFor(ctx), (db) => orgService.decideKyb(db, {
                organizationId: ctx.params['organizationId'],
                decision: field(body, 'decision', oneOf('approve', 'reject', 'request_information', 'escalate', 'suspend'), 'a decision'),
                reason: field(body, 'reason', isNonEmptyString, 'a written reason of at least 20 characters'),
                userId: user.userId,
    ...
    router.register({
        ...
        tags: ['compliance'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), async (db) => {
            const active = await many(db, `SELECT rule_key, version, name, category, severity, on_trigger_action, parameters,
            ...
            const triggerCounts = await many(db, `SELECT rule_key, count(*)::text AS n FROM rule_evaluation
            WHERE triggered GROUP BY rule_key`);
            const counts = new Map(triggerCounts.map((r) => [r.rule_key, Number(r.n)]));
            return {
                rules: active.map((r) => ({ ...r, times_triggered: counts.get(r['rule_key']) ?? 0 })),
                catalogue_size: RULES.length,
                note: 'Rules are immutable once published. A change creates a new version; historical evaluations ' +
    ...
    router.register({
        ...
        tags: ['compliance'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => orgService.expiringDocuments(db, Number(ctx.query.get('within_days') ?? 30))),
    });
    // -------------------------------------------------------------------------
    // Ledger and finance
    // -------------------------------------------------------------------------
    router.register({
        ...
        tags: ['ledger'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => ledger.accountBalances(db, {
    ...
    router.register({
        ...
        tags: ['ledger'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => ledger.journalsForTransaction(db, ctx.params['id'])),
    });
    router.register({
        ...
        handler: async (ctx) => {
            ...
            return withContext(contextFor(ctx), (db) => ledger.reverse(db, { journalId: ctx.params['id'], reason, postedBy: user.userId }));
        },
    });
    // -------------------------------------------------------------------------
    // Reconciliation
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            const dateParam = ctx.query.get('business_date') ?? ctx.body?.['business_date'];
            const businessDate = dateParam ? new Date(String(dateParam)) : new Date();
            return withContext(contextFor(ctx), (db) => recon.runDailyReconciliation(db, businessDate, user.userId));
        },
    });
    router.register({
        ...
        tags: ['reconciliation'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => recon.listRuns(db)),
    });
    router.register({
        ...
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), async (db) => {
            ...
            if (!run)
                throw notFound('RUN_NOT_FOUND', 'Reconciliation run not found.');
            return run;
        }),
    });
    router.register({
        ...
        tags: ['reconciliation'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => exceptions.listExceptions(db, {
    ...
    router.register({
        ...
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), async (db) => {
            ...
            if (!exc)
                throw notFound('EXCEPTION_NOT_FOUND', 'Exception not found.');
            return exc;
        }),
    });
    router.register({
        ...
        handler: async (ctx) => {
            ...
            return withContext(contextFor(ctx), async (db) => {
                const exc = await one(db, 'SELECT id FROM exception_case WHERE reference = $1', [ctx.params['reference']]);
                await exceptions.addExceptionNote(db, {
                    exceptionId: exc.id, authorId: user.userId, body,
                    evidenceRefs: ctx.body['evidence_refs'] ?? [],
                });
                return { added: true };
            });
        },
    ...
    for (const definition of reports.REPORT_DEFINITIONS) {
        router.register({
            ...
            handler: async (ctx) => {
                ...
                const result = await withReadOnlyContext(contextFor(ctx), (db) => reports.run(db, definition.key, filters, maskingProfileForRoles(user.roles)));
                if (format === 'json')
                    return result;
                const rendered = format === 'csv' ? { body: toCsv(result.columns, result.rows), contentType: 'text/csv; charset=utf-8', ext: 'csv' }
                    : format === 'xlsx' ? { body: toXlsx(definition.title, result.columns, result.rows), contentType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', ext: 'xlsx' }
                        : format === 'pdf' ? { body: toPdf(definition.title, result), contentType: 'application/pdf', ext: 'pdf' }
                            : null;
                if (!rendered)
                    throw invalid('UNSUPPORTED_FORMAT', 'Supported formats: json, csv, xlsx, pdf.');
                await withContext(contextFor(ctx), (db) => reports.recordExport(db, {
    ...
    router.register({
        ...
        handler: async (ctx) => {
            ...
            return reports.REPORT_DEFINITIONS
                .filter((d) => user.permissions.has(d.permission))
                .map((d) => ({
    ...
    router.register({
        ...
        tags: ['audit'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => many(db, `SELECT a.seq::text AS seq, a.id, a.occurred_at, a.category, a.action, a.outcome,
    ...
    router.register({
        ...
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), async (db) => {
            ...
            return {
                ...result,
                method: 'Each entry stores the hash of its predecessor and a hash over its own contents. ' +
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            return withReadOnlyContext(contextFor(ctx), async (db) => {
                const rows = await many(db, `SELECT seq::text AS seq, id, occurred_at, category, action, outcome, actor_role,
            ...
            ORDER BY seq LIMIT 10000`, [ctx.query.get('from_seq'), ctx.query.get('to_seq')]);
    ...
    router.register({
        ...
        handler: async (ctx) => {
            const user = requireUser(ctx);
            return withReadOnlyContext(contextFor(ctx), (db) => notify.inboxFor(db, user.userId, user.roles));
        },
    });
    router.register({
    ...
    router.register({
        ...
        tags: ['cases'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => many(db, `SELECT s.reference, s.category, s.subject, s.priority, s.status, s.opened_at,
    ...
    router.register({
        ...
        handler: async (ctx) => {
            ...
            return withContext(contextFor(ctx), async (db) => {
                ...
                const row = await one(db, `INSERT INTO support_case (
           ...
           RETURNING id`, [
                    ...
                    priority, user.userId, body['transaction_id'] ?? null, String(slaHours),
                ]);
                if (category === 'complaint') {
                    await db.query(`INSERT INTO complaint (support_case_id, organization_id, complaint_type)
             VALUES ($1,$2,$3)`, [row.id, user.organizationId, body['complaint_type'] ?? 'service']);
                }
                await recordAudit(db, {
    ...
    router.register({
        ...
        tags: ['admin'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), async (db) => {
            const config = await many(db, `SELECT config_key, version, value, value_type, description, is_placeholder,
                  founder_decision_ref, status, effective_from
             FROM system_configuration WHERE is_current ORDER BY config_key`);
            const flags = await many(db, 'SELECT key, description, enabled, is_release_gate, is_immutable FROM feature_flag ORDER BY key');
            const corridors = await many(db, `SELECT code, origin_country, destination_country, origin_currency, destination_currency,
            ...
            return {
                ...
                note: 'Configuration values are immutable once written. A change is a new version under ' +
    ...
    router.register({
        ...
        tags: ['learning'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => learning.productMap(db)),
    });
    router.register({
        ...
        tags: ['learning'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => learning.glossary(db)),
    });
    router.register({
    ...
    router.register({
        ...
        handler: async () => ({
            ...
            note: '"Settled" means the partner reported the payment as made. It does not mean settlement ' +
    ...
    router.register({
        ...
        tags: ['learning'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => learning.decisionLog(db)),
    });
    router.register({
        ...
        handler: async (ctx) => {
            ...
            return withContext(contextFor(ctx), (db) => learning.approveDecision(db, ctx.params['ref'], user.fullName, reason, user.userId));
        },
    });
    router.register({
        ...
        tags: ['learning'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => many(db, 'SELECT * FROM build_journal_entry ORDER BY entry_date DESC, milestone')),
    });
    router.register({
        ...
        tags: ['learning'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => many(db, `SELECT risk_ref, category, title, description, inherent_likelihood, inherent_impact,
    ...
    router.register({
        ...
        tags: ['learning'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => learning.walkthrough(db, ctx.params['transactionId'])),
    });
    router.register({
        ...
        tags: ['learning'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => learning.assessment(db, ctx.params['moduleKey'])),
    });
    router.register({
        ...
        handler: async (ctx) => {
            ...
            return withContext(contextFor(ctx), (db) => learning.submitAssessment(db, ctx.params['moduleKey'], user.userId, answers));
        },
    });
    // -------------------------------------------------------------------------
    // Regulator view
    ...
    router.register({
        ...
        tags: ['regulator'],
        handler: async (ctx) => withReadOnlyContext(contextFor(ctx), (db) => reports.regulatorOverview(db)),
    });
    // -------------------------------------------------------------------------
    // OpenAPI
    // -------------------------------------------------------------------------
...
export function buildOpenApi(router) {
    const paths = {};
    ...
    return {
        ...
        info: {
            ...
            version: '0.1.0',
            description: 'B2B cross-border trade settlement orchestration. This deployment settles through simulators ' +
        ...
        components: {
            securitySchemes: {
                sessionCookie: {
                    ...
                    description: 'Opaque session token. State-changing requests must also present the CSRF token from the ' +
